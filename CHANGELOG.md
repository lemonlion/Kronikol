...
- **Every run now says where its reports are and how to debug them, and leaves two files behind that say it again.** An agent watching a failing build had no signal that `kronikol query` exists: the reports directory was never printed by the library path, so the only discoverable artefact was a 10 MB JSON file that cannot be read. Four things close that loop, all on by default.
  ...
  - **This was measured before it was designed.** The assumption that a library's `Console.WriteLine` from a run-end hook reaches the CI log is **false**: on .NET 10, `dotnet test` swallows it for xUnit v2, xUnit v3, NUnit 4 and MSTest at default verbosity *and* at `-v normal`, which is what CI runs; it appears only at `-v detailed`, or under `dotnet run --project` on a Microsoft.Testing.Platform host - and TUnit's runner suppresses it even there. So the files are the channel and the console is the bonus, which is the opposite of how this feature was originally scoped. The CI job summary is unaffected because it is written to a file descriptor rather than to stdout, which is why the debug section goes there too. The full per-runner table is in `LLM_FRIENDLY_PLAN.md`.
...
- New repo-root `LLM_FRIENDLY_PLAN.md` — the plan for making Kronikol the tool an agent debugs with. This release is its M0 (ride-along fixes) and M1 (the discovery loop); its §11 records what shipped, the decisions taken during implementation and the measured per-runner console table, and a `PLANS_STATUS.md` row tracks it.
...
- `DIAGRAM_WIDTH_PLAN.md` and `NOTE_WRAP_AND_WIDTH_PLAN.md` are committed as executed design records.
...
### Added
- **The report search box now searches *everything* the report contains** (SEARCH_INDEX_PLAN.md, executed in full). Until now the search deliberately covered only a per-scenario summary (names, steps, error messages, extracted participant names and URLs) — a string you could *see* in a note payload, a diagram message, an SQL statement or a flame chart was unfindable. Reports now embed a compact full-text index (`<script id="kron-search-index">`: hashed-trigram → scenario-bitset, FNV-1a-32 over the normalized corpus, adaptive bitset/delta-varint rows, gzip+base64) over every diagram's full source, note payloads and headers, flame-chart span/source/marker text, and parameterized example values. **Measured: a 147 MB-of-diagram-text monster report gains a 1.07 MB blob** (typical reports gain far less; `tools/search-bench/` holds the full measurement record).
