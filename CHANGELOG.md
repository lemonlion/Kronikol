...
from 320 px up, the scenario toolbar is styled in every report, and a custom stylesheet wins over the
built-in ones.** The patch part moved because nothing is new to call: no option, type, member or
parameter is added. `InternalFlowPopupCustomStyleSheet` has existed since internal-flow tracking and is
now applied; everything else is CSS and the order of the stylesheets in the report's head. Every
behaviour that changes is one that was broken, and each is called out below.

### Fixed

- **The report scrolled sideways between 770 and about 1080 px.** Beside the summary and the CI box the
  filtering box was squeezed to 32 px at 770 px (125 at 900, 225 at 1000) and its export buttons ran
  out of it, 295 px past the window's edge at 770 px on a report with CI metadata. From 769 to 1160 px
  the filtering box now takes a row of its own under the summary and the CI box. **Behaviour change:**
  in that band the header is two rows, with empty space beside the summary; at 1161 px and up, and at
  768 px and below, it is laid out as before. The band starts at 768.02 px, not 769, so a fractional
  viewport (Firefox gives a 769 px window 768.8 CSS px at 125 % zoom) cannot fall between it and the
  phone layout.
- **A long branch or repository name scrolled the page sideways from 780 to 1320 px.** The CI box never
  shrank and its cells never wrapped, so a 69-character branch made it 547 px wide. Above the phone
  layout the box is now at most 20 em wide and its values wrap, at any character if they must; its
  labels stay whole. A short branch is unaffected.
- **The export buttons wrapped their labels inside themselves**, at 380 px and below and from 770 to
  1260 px on a report with CI metadata. The labels no longer wrap: the buttons wrap as whole buttons,
  and drop under the "Filtering" heading when the box cannot hold both.
- **The top bar's buttons wrapped their labels between 500 and 580 px** on every run report with the
  default component diagram, whose "Component Diagram" button made a fourth in a row that wrapped only
  at 480 px and below. The buttons now wrap whole.
- **A scenario toolbar with many controls cut its last ones off, out of sight.** With internal-flow
  tracking the toolbar was a flex row that never wrapped: its labels wrapped inside their buttons and,
  because a scenario clips whatever overflows it (`content-visibility: auto`), its trailing controls
  were cut off with nothing to scroll to, from 780 to about 1300 px on the published BreakfastProvider
  reports, up to 522 px of them. The toolbar now wraps its controls onto a new row. **Behaviour
  change:** a toolbar too wide for its line puts the controls that do not fit (the Details buttons, the
  toggles, the note selects) at the left of the next row, as it already did at 768 px and below.
- **Without internal-flow tracking the scenario toolbar was unstyled.** Its rules lived in the
  internal-flow stylesheet, which only flow-tracking reports carry, so everywhere else it was a plain
  block. They are in the main stylesheet now. **Behaviour change:** in those reports the toolbar has
  16 px of side padding and lays its controls out as a wrapping row.
- **A custom stylesheet lost to the built-in component stylesheets.** `HtmlSpecificationsCustomStyleSheet`
  was emitted before the context-menu, inline-SVG, notes and internal-flow sheets, so every rule it
  gave a control those sheets style lost at equal specificity: on every default `Specifications.html`
  the active Details button was blue in a violet report, and with internal-flow tracking so were the
  active diagram tab, the popup's flow toggle and its related-tests list. The custom sheet now follows
  the built-in sheets and precedes `CustomCss`. **Behaviour change:** a user stylesheet's rules for those
  controls now take effect, as the option's name always said. A report without a custom stylesheet
  gains no byte from the move: on six such report shapes generated by 3.29.1 and by this release, the
  only difference is the changed stylesheet rules.
- **On the violet theme an active filter lost its colour while hovered**, and on a phone, where a tap
  leaves the control hovered, the filter just tapped showed white text on pale lavender until the next
  tap elsewhere. The theme stated its hover rules after its active rules; the hovers now come first.
  The theme also gains the hover and active colours it lacked, which painted blue on a violet page: the
  dependency and category AND/OR toggles, the percentile, collapse/expand-all, timeline and export
  buttons, the Details buttons, the phone "Diagram Settings" button, the diagram tabs, the popup's flow
  toggle and related-tests table, the scenario "#" link and copy button, and a search-matched
  parameterized row's marker.
- **`InternalFlowPopupCustomStyleSheet` was never applied.** It was documented, and read by nothing.
  With internal-flow tracking on, both HTML reports now carry it after the specifications theme and
  before `CustomCss`. **Behaviour change:** a consumer who set it now gets it.

### Documentation

- The doc comments of `HtmlSpecificationsCustomStyleSheet`, `InternalFlowPopupCustomStyleSheet`,
  `CustomCss` and `Stylesheets.VioletThemeStyleSheet` say where each sheet goes. Wiki:
  `Report-Configuration` no longer says the specifications stylesheet "replaces the default stylesheet"
  (the base sheet is always emitted, and the default is a violet overlay on it), and with
  `Internal-Flow-Tracking` says the popup stylesheet is applied from 3.29.2; `API-Reference` says what
  `VioletThemeStyleSheet` is; `Generated-Reports` says how the report lays out by window width. The
  Kronikol4J divergence ledger records the change.

### Tests

- `ViewportSweepTests`: every width from 320 to 1400 px in 20 px steps plus 769 and 1161, reloaded at
  each, in a Chromium of its own that draws classic scrollbars (Playwright's default launch hides
  them), on three pages with the widest header and the fullest scenario toolbar a report can have. At
  each width: no sideways scroll, no label wrapped inside a toolbar button, the export buttons inside
  their box, every scenario-toolbar control inside its toolbar (the page's scroll width cannot see a
  clipped one), and the header laid out as the breakpoint says. On 3.29.1 it reports 280, 1,050 and
  1,315 problems on its three pages.
- `ToolbarStylingTests` reads what each toolbar state paints on the violet and on the blue report, taps
  a filter on an emulated phone, and applies the popup stylesheet; `StylesheetRulesTests` and
  `ReportGeneratorStyleOrderTests` pin where each rule lives, the 768.02 px bound, the theme's order and
  the stylesheets' order. `MobileResponsiveTests.Diagram_toggle_wraps_on_mobile` read its toolbar while
  the phone layout hid it, so it could not fail; it now opens the toolbar first. `DiagramToggleLayoutTests`
  polls at 200 ms, as every E2E wait must. Three older facts that matched stylesheet text as a substring
...
- **One report grouped its failures two ways.** `Failures.md` and the HTML report's cluster panel each had their own implementation of "the first line, normalised", and they disagreed three ways, all reachable: the digest cut at the first CR **or** LF while the panel split on LF alone (so a bare-CR message keyed the whole message on one surface and its first line on the other); the panel admitted a present-but-empty message and formed a cluster headed by a blank line where the digest formed none; and the two used different definitions of whitespace. There is now one function, and the tests compare the two surfaces against **each other** rather than against a literal — a literal can only pin what one of them does.
- **The HTML report labelled the error message `Failure Cause:`.** True by coincidence on xUnit v3, where the adapter had spliced the cause onto the message, and a mislabel on the other seven adapters, where the text after the label is the assertion. The message is now labelled `Error:` and the cause gets its own `Cause:` line when the framework supplied one.
- **The digest's "N further failures" table repeated the heading instead of describing the failure.** That column is the only thing a reader is told about a failure that was clustered away, and it printed the failure's first line — which is the line the clustering has just established every member of the cluster shares. It now flattens the whole message, which is what `kronikol query failures` has always done, and is why the two surfaces could show one run as one cause and as four.

### Tests
- **Every component-diagram width bound on CI was measured against a 400-pixel error card.** `ComponentDiagramLabelWidthTests` exists because a user-reported architecture overview drew 6697px wide and was silently cropped to 4096, taking every dependency node off the canvas. The CI runner has no Graphviz; PlantUML without `dot` returns a valid SVG of an error card rather than failing; and the suite's render helper guarded only `Assert.DoesNotContain("Syntax Error", svg)`. So from the commit that introduced those facts (3.0.83) until now, **every width assertion in that class passed against an image with no diagram in it**, and the one fact that looked at the drawn text was the only one that noticed — which is why a single test had been red on CI for nine runs while its five siblings were green for the wrong reason. All renders now go through a helper that rejects PlantUML's own error images outright, naming the phrase that gave it away and what to install; `Every_dependency_node_survives_the_reported_diagram` now checks that the nodes actually survive, which is what its name always promised and which a raster width could never show. Proved both ways in a Linux container: with no Graphviz the component facts fail loudly with that message, and with Graphviz installed all 49 pass. The CI job installs it, because the bound should be measured against the engine the reported defect happened under rather than against the fallback.
- **`StableIdDeepLinkTests` in the E2E project asserted against ids no report had written since 3.1.0.** It recomputed each expected id with `ScenarioStableId.Compute(null, …)` while `GenerateHtmlReport` resolves the suite itself, so the element it looked for was simply not in the document and Playwright said `Cannot read properties of null` seven times. The unit-test copy of the same class had already been corrected; the E2E copy is a different file in a different project and was missed. It now resolves the suite the way the generator does.
- **`The_html_and_the_data_file_carry_the_same_ids`** — the guard that would have caught the above without a browser. A `#sid-` link crosses between the two artifacts: `Failures.md` and `kronikol query` read an id out of the data file and hand it to the HTML. Nothing compared them; both suites compared one artifact against a recomputation, which cannot see the two drifting apart because each stays internally consistent while doing it. Mutation-proved: removing the suite resolution from the HTML writer reddens it.
...
release where the rule bites: it adds new public surface in quantity, and the highest-ranking change
decides. New emitted files (`Failures.md`, `Failures.jsonl`, `CLAUDE.md`, `AGENTS.md`), five new fields
in every data file (`ciMetadata`, `environment`, `feature.sourceFile`, `scenario.sourceFile`,
`scenario.sourceLine`, `scenario.attempt`), a new public record (`RunEnvironment`), a new
`DiagnosticKind` member (`ResultDefaulted`), a new URL-hash form (`#sid-<stableId>`), and new output
from four `kronikol query` verbs. The bug fixes travel with it and do not lower the bump.

### Fixed
- **`kronikol query --json` answers with an envelope on every exit code, not only on success.** A non-zero exit printed prose on stderr and *nothing at all* on stdout — the whole buffered answer, provenance banners included, was discarded by the one line that flushes it. So a wrapper asking for `--json` got valid JSON when the call worked and an empty string when it did not, which is the single case a machine channel exists to handle. A failure now carries `error: {exitCode, message, hint}` with an empty `items` array, and the prose still goes to stderr unchanged. The hint is everything the tool said after the first line — the legal values for a flag, the verbs that support a format — which is the half a caller can act on. It works from the earliest possible failure: a report path that does not exist answers with `report: null`, `kronikolVersion: null` and the message, rather than with silence.
- **`truncated` says how many rows were dropped.** It was a bare boolean, so a consumer learned that the budget had bitten and never how hard — which is the silent-skip problem re-encoded rather than fixed. It is now `{limitBytes, droppedItems}`, or `null` when nothing was dropped, so the presence of the key is itself the signal. And **`total` is always present**, null when the verb does not page: `summary` and run-`diff` omitted the key entirely, and a consumer that has to branch on key *presence* cannot tell "this verb does not page" from "an older tool wrote this" — the same rule `kronikolVersion` is deliberately present-and-null to protect.
- **`Failures.md` numbered its scenarios with a different comparer from the report it points into.** The digest's `sN` addresses exist so a reader can go straight to `kronikol query s3`, and its own comment says they must match the tool's - but `FailuresDigestGenerator.Enumerate` ordered features with `StringComparer.Ordinal` while `BuildFeaturesJsonModel`, and therefore `TestRunReport.json`, and therefore `kronikol query`, orders them with the default culture-sensitive comparer. Ordinal sorts every upper-case initial ahead of every lower-case one, so a suite with an `Order API` and an `Order api` numbered them one way in the digest and the other way in the file: `s1` in `Failures.md` was `s2` to the tool, and every address after the first divergence pointed at a scenario that had not failed. The digest now uses the same bare `OrderBy(f => f.DisplayName)` as every other writer. Only reports whose feature names differ by case, punctuation or diacritics are affected - on ASCII-alphanumeric names the two comparers already agreed, which is why this survived. The existing cross-check could not see it: it uses a single feature, and one feature sorts the same under any comparer.
- **`kronikol query --json` handed back a resume pointer that fell apart on any filter value with a space in it.** `next` was one interpolated string, so `--service "Dessert Provider"` came back as `--service Dessert Provider --json --offset 3`; a consumer that split it on whitespace passed `Provider` as a positional and got `Not an address: Provider` — with no envelope, because errors are plain text on stderr. No quoting convention fixes that, since the consumer decides how to split, so **`next` is now an argument vector**: `["query","interactions","<report>","--service","Dessert Provider","--json","--limit","3","--offset","3"]`. It is the *whole* command rather than the flags to append — the verb, the resolved report, every positional address and every filter — so a script needs to remember nothing about the call it is resuming, and two things that were silently wrong are fixed on the way: the pointer now carries the **positional addresses** (`interactions s0 --json` used to resume across the entire report, the same envelope shape with a quietly wider answer) and the **page size** (page two came back at the default width, so a pointer that "resumed" returned a different-sized page than the one it followed). The text footer is unchanged in content and now quotes values that need it, both formats rendering the same tokens so they cannot disagree. `formatVersion` stays `1`: no released version ever emitted the string form.
- **`kronikol query` counted Kronikol's own success labels as errors.** The classifier knew the HTTP non-200 successes by name and treated every other non-numeric status as a failure — but the labels Kronikol itself stamps on calls that have no status code are exactly that: a broker publish is `Sent`, a consume `Ack`, a reply `Responded` (`MessageTracker`'s own defaults), a cache lookup `Hit` or `Miss`, a Spanner transaction `Committed`. So a clean run of a message-driven service reported one error per call — `services` printing `Event broker  8 calls  8 errors  …  Responded×8`, a 100% error rate invented out of its own vocabulary — and `flow --errors-only` and `--group-by` agreed with it, because all three ask the one classifier. Those six labels are now known successes; a refusal (`Nack`, `Fault`) is deliberately still an error. A cache **miss** is an outcome, not a failure.
- **`--slower-than` was rendered into the rerun pointer in the current culture**, so on a locale with a comma decimal separator a paged re-run carried `--slower-than 5,5` — which the parser rejects. Invariant now, like every other number the tool prints.
- **Indexing a report allocated a UTF-16 copy of every payload in it, then a UTF-8 copy of that** (found by the streaming test this release adds, which is the only test that could ever have seen it). `ReportScanner` streams the file through a windowed reader and keeps byte offsets, never payload text — that is the design the whole tool rests on, and it was true of everything except the one branch that matters most by volume. On each `content` token it called `reader.GetString()` to get two numbers, a character count and a SHA-1, and `HashBody` then called `Encoding.UTF8.GetBytes` on the result; the text was dropped immediately afterwards, because what the index stores is the range. **Scanning the 142 MB `tools/query-bench` corpus allocated 324 MB — more than twice the file — before a single question had been answered**, and the cost tracked payload bytes rather than interaction count, which is precisely the property "streaming" denies. The hash is over the same bytes the reader is already sitting on, so the unescaped case now hashes `ValueSpan` directly and counts characters without decoding, and the escaped or window-split case decodes once into pooled buffers. **324 MB → 32 MB on the 142 MB corpus, 13 MB → 1 MB on a 6 MB one.** Every `b:` address is unchanged — the hash input is byte-for-byte what it was, which the existing body-hash pins prove. Nothing about the answers changed; this is what producing them costs.
- **A paged listing could hand back a `next:` pointing at the page you were already on.** `QueryWriter.Page` treated "finished" as `last >= all.Count && offset == 0`, so the two ways a page can render nothing both fell through to the resume branch and printed `next: --offset {the offset just used}`: an `--offset` past the end of the listing, and a first row too large for the byte budget. A reader shrugs and raises the budget; a script that follows `next` re-runs the identical command forever. It also printed nonsense ranges — `scenarios: 100-99 of 7`. There are now three endings and they are told apart: past the end says `scenarios: nothing at --offset 900 · 127 scenarios in total`, a row that cannot fit says `calls: nothing fits — row 1 is larger than 200 bytes · raise with --max-bytes`, and a listing exhausted by paging says `scenarios: 41-127 of 127` — none of them offering a next page. Under `--json` `next` is `null` in all three cases, with a note explaining the budget one.
- **`kronikol query failures` ignored `--limit` in its footer arithmetic.** The verb hand-rolled its own `Skip`/`Take` and its own footer instead of going through the one pager, and that footer hard-coded 25 — the default cap — so `--limit 2` against three failures printed two of them and then reported `3 failed` with no `--offset` to resume from. The third failure was unreachable and nothing said so. `failures` pages through `QueryWriter.Page` now, like every other listing, which fixes the arithmetic and gives it the `next:` contract for free; its closing line is unchanged when the listing is exhausted.
- **`kronikol query <verb> --sort X` accepted the flag and discarded it on every verb but two.** 3.1.0 already fixed this for `interactions`; the same silence remained everywhere else. Only `services` and `interactions --group-by` order their rows — `scenarios --sort duration`, which is the one an agent hunting a slow test will reach for, sorted nothing and said nothing, so row one read as the slowest when it was merely the first in the report. Every verb that cannot apply `--sort` now exits 2 and names the two that can.
- **`kronikol query diff` swallowed the one message it prints when a body cannot be read back.** It wrote "A body could not be read back from the report." through the `QueryWriter` and returned 1 — and the writer is flushed only on exit 0, so the buffer was discarded and the command failed silently with no output at all. It goes to stderr, where the rest of the tool's failures already go.
- **Export Filtered HTML shipped no diagrams** (user-reported, found while investigating a report whose diagrams would not render) — the exported `filtered-report.html` contained the whole rendering machinery and none of the diagram data, so every diagram in it was permanently blank. Under `BrowserJs` rendering a diagram element is an empty `<div class="plantuml-browser" id="puml-N">`; the sources live gzipped in one `<script id="puml-data">` keyed by element id, and that script is the last thing in the **body**. `export_html` copied `document.head.innerHTML` and the visible `details.feature` elements and nothing else — so `getPumlZ` returned null for every element, `enqueueElement` silently did nothing, and the only diagrams that appeared were the handful already drawn before the export, serialised as inline SVG. There was no error box and no console message: the boxes were simply empty. Measured against a real 488-diagram report, exported with a dependency filter applied: **12 of 460 diagrams drew, 448 did not**; after the fix, **460 of 460, none orphaned**. The export now appends the body's data payloads (the only two scripts there, both `application/json`, so nothing the head already ran can re-run), with `#puml-data` pruned to the ids the exported markup actually contains — safe to the letter because `_pumlData` has exactly one reader, `_pumlData[el.id]` in the render script's `getPumlZ`, so an id absent from the export can never be asked for. The real report's export goes from 7.13 MB to 9.99 MB. Tests: a new `ExportFilteredHtmlRenderingTests` opens the exported file in a browser and waits for an `<svg>`, asserts the payload script is present, and asserts no container is left unable to resolve a source. The pre-existing coverage could not see any of this — `Export_filtered_html_carries_the_seeded_defaults` asserts that a seeded script string appears in the exported text, which it always did, because the head is copied verbatim; presence of the machinery says nothing about whether a diagram renders. Same bug class as the 3.0.82 `component-diagram-section` substring facts.
- **`kronikol query diff <old.json> <new.json>` no longer throws on a repeated `stableId`** (found while re-verifying `plans/LLM_FRIENDLY_PLAN.md`) — the cross-run diff keyed both reports with `ToDictionary(s => s.StableId)`, and a `stableId` is not unique: a `[Theory]` with repeated data, the same example row in two `Examples:` blocks, a retried scenario in a mergeable file or an ingested repeat all give two scenarios one id, and the command died with `ArgumentException: An item with the same key has already been added` before printing a line. Scenarios sharing an id are now grouped and matched in file order, the diff announces it (`! N scenarios share a stableId (repeated rows or retries) — matched in order`), and a repeat the new run no longer has counts under `Gone`. `diff --body s3/i47` across runs used `FirstOrDefault` and silently took the first holder; it now matches the n-th holder in the old report to the n-th in the new.
- **`TestRunReport.schema.json` declares every field the writer emits** — `exampleFlatValues` and `exampleDisplayName` have been written since the merge work but were never declared, so the file the wiki calls "the field-level contract" had drifted from the data. Both are declared now, and a new walker test (`Every_key_the_json_writer_emits_is_declared_in_the_schema`) generates a report through every emitter — example rows, attachments at both levels, background steps, parameters of all three kinds, text segments, doc strings, comments, bypass reasons, assertions with source locations, interactions with headers and W3C ids, Row/Custom annotations, diagnostics — and fails on the first undeclared key, so a field can no longer land in the writer alone.
- **Adapter-driven runs record their diagnostics** — `ReportDiagnosticsScope` was opened in exactly one place, `kronikol ingest`, so on every framework-adapter path (xUnit, NUnit, MSTest, TUnit, ReqNRoll, LightBDD, BDDfy) each `ReportDiagnosticsScope.Record` — render failures, attachment failures, step-attribution mismatches, output failures — was a silent no-op: `TestRunReport.json`'s `diagnostics` array was always `[]` and the HTML's *Report diagnostics* block never appeared, whatever went wrong. `CreateStandardReportsWithDiagrams` now opens a collector when none is scoped; a host's own scope (ingest, a dashboard) is reused rather than replaced, so those paths are unchanged, and the `AsyncLocal` keeps two concurrent generations apart. The console lines were never affected — only the structured channel was dead.
- **A paging footer told an agent to run a command that produced a different list.** `kronikol query`'s `next:` line is the whole contract behind "if you did not see a footer, you saw everything" — and `RerunPrefix` carried the filters but not `--sort`, `--in`, `--values`, `--step` or `--slower-than`, while `grep`'s footer did not call it at all. So `interactions --group-by service,status --sort errors --limit 2` handed back `--offset 2` without the sort: the second page re-bucketed in the default order, reprinted rows already shown, and announced `3-4 of 4` — the two largest buckets never appeared and the agent was told it had seen everything. `grep "x" --in uris --limit 3` was worse: the continuation searched the default four targets instead of the one asked for, so the total changed underneath the reader and the hits it was still owed were unreachable. Both carry their flags now.
- **`kronikol query grep --in ""` searched nothing and said so as though it were an answer.** An empty, comma-only or whitespace value is not null, so the default target set did not apply, and the split produced no targets at all — which the new unknown-target guard passed by never entering its loop. The emptiest possible input defeated the check whose whole purpose is that grep never silently searches nothing. Zero targets is now an error.
- **`kronikol query interactions --sort X` accepted the flag and discarded it.** Nothing reads `--sort` on the ungrouped path; the list is capture order, always. An agent asking for `--sort duration` to find the slow call read row one as the slowest when it was merely the first captured — and now that the two views which *can* sort refuse a value they cannot apply, silence here was the stronger wrong signal. It exits 2 and names `--group-by` and `services --sort` as the two things that do sort.
- **A merged report claimed every scenario had made no calls.** The "no interactions captured for this scenario" affordance is gated on the tracked logs, and the merge renderer passed none — so the check fell through to `RequestResponseLogger.RequestAndResponseLogs`, the ambient log of the *merging* process, which is always empty. Every merged scenario carrying a sequence diagram was therefore labelled as having touched nothing, directly contradicting the diagram printed above the label. The merge now passes the traffic it carries. Every other consumer of those logs in the HTML generator already fell back to the precomputed diagram text, which is why this was the only visible casualty.
- **Merging a sharded run silently threw away every step's failure message and every step's source location** (found while adding scenario-level locations). Both step mappers have written `failureMessage`, `sourceFile` and `sourceLine` since 3.0.47; `MergeableReportReader.ReadSteps` read none of the three. A merged report therefore showed the step that failed without saying why or where — the detail that makes a merged report worth reading — and nothing in the output said anything was missing. All three are read now, and a fact pins the round trip.
- **`kronikol query diff` no longer accuses a pre-3.0.47 report of a stableId collision.** A report written before stable ids existed carries none at all, so every scenario fell into the empty-string group and the M0 duplicate warning announced that every scenario in the file "shares a stableId (repeated rows or retries)". It now says what is actually true: `! this report has no stableIds (written before 3.0.47) — scenarios matched by position`. The repeated-id count skips the empty group.
- **`kronikol query --raw` removed** — parsed and never read since 3.0.47 (`PLANS_STATUS` item 6, decision: delete); it is now the usual `Unknown option: --raw` usage error, like any other unknown flag. **Deliberately shipped in a minor rather than held for v4**, which is where removals normally go: the flag never did anything, so nothing that passed it was getting a behaviour worth preserving, and keeping an inert flag alive for a whole major cycle to protect a no-op is not a trade worth making. It is a breaking change to exactly one thing — a script that passes `--raw` now exits non-zero on the usage error instead of succeeding while the flag was ignored. Drop the flag; nothing else changes.
- **Provenance banner wording** — `! mergeable report (a merge of several runs)` said the wrong thing: `mergeableFormatVersion` marks the superset *format* every runner writes for `kronikol merge` (`GenerateMergeableData = true`), not the output of a merge. It now reads `! mergeable-format report`.
- **`kronikol query` enforces per-verb flag legality — a filter that cannot run is refused rather than ignored.** `QueryOptions.Parse` was one flat, verb-agnostic switch: it rejected an *unknown* flag but never asked which verb it was parsing for, so every flag the tool has was accepted by every verb the tool has. The worst shape was `failures <report> --service payments`, which printed every failure in the report — an answer indistinguishable from "no call to payments broke anything", which is the one conclusion it does not support. `scenarios --status 500`, `summary --limit 3`, `steps --failed`, `flow --grep X`, `interactions --errors-only` and `values --headers` were the same defect wearing different flags. Each verb now declares what it reads; anything else exits 2 naming the flag, the verbs that *do* read it, and what this verb accepts. Two flags keep their own bespoke refusals, which were the only per-verb checks that already existed: `--sort` and `--json`. The table is held to the parser from both sides — a flag it names that the parser does not have fails a test, and so does a flag the parser has that no verb can use — and a third test puts every command line in the skill reference through the same table, so the documentation cannot tell an agent to type something the tool refuses.
- **`kronikol query http` and `body` each accept the other's address.** They took the two address kinds a listing prints and each refused the other's: `body s4/i8` answered "Which body?" and `http b:29b82e93` answered "Which interaction?", though both addresses were printed side by side on the row the reader was looking at. `body` now takes a call address (and, when that call carried no body, says which half of the pair did), and `http` takes a content address, naming every call that carries it and describing the first.
- **A body's address now leads back to a call the listing printed.** `interactions` folds a request and the response that answered it into one row under the **request's** address, so a response's own address appeared in no listing anywhere — yet that is the address `body` reported, and the address `grep` prints for a hit. `body b:9a3f25b7` would answer `at 1 address(es): s4/i11` when the listing had shown that body beside `s4/i8`, and nothing connected the two. `http` now prints the address of the call's other half (`response s4/i11` on a request, `answers s4/i8` on a response), and the occurrence list marks a response for what it is: `s4/i11  (the response to s4/i8)`. Every address the tool prints is now reachable from a listing, which is what "each command ends with the addresses that fetch the next thing" was supposed to mean.

### Added
- **Every data file declares its own shape version, and every reader gates on it.** `TestRunReport.json`, `.xml` and `.yml` now open with `formatVersion: 1` — first key, first element, first line — declared in the JSON Schema as required and in the XSD as the first element of the sequence. It versions the *shape*, which is a different fact from `kronikolVersion`: that moves on every release whether or not anything changed, and every working-tree build stamps the same string, so it could never answer "can this reader read this file". Additive changes do not move it; a key changing meaning or type does, which is exactly when a reader has to stop rather than continue. `kronikol query` and `kronikol merge` now refuse a version they do not understand instead of half-parsing it. The same gate closed a hole in `mergeableFormatVersion`: a version that was present but was a string, a float or null landed as `null` in the index, which is indistinguishable from a file with no version at all, so the unknown format was accepted silently — and `merge` then **re-stamped its output as version 1**, laundering it past the gate every reader downstream applies. Present-but-unreadable and absent are now different answers, and only the second one is allowed through. It lands before any v1 file exists without it, so no reader ours or the port's has to carry a "version 1 means absent" branch for ever.
- **`stableId` is scoped to the suite that produced it**, and the suite is written into the report so the id can be explained. Nothing on the model carried one: a `Feature` has a display name, a `Scenario` has no project or assembly, and the only other "suite" in the codebase is CTRF's, which is defined as the feature name again. So it is resolved — from the single `*.deps.json` beside `AppContext.BaseDirectory`, which under `dotnet test` is the test project's own output directory. Deliberately **not** `Assembly.GetEntryAssembly()`, which under VSTest is `testhost` and would scope nothing, and **not** `GetCallingAssembly`, which the LightBDD adapters defeat by reaching generation through a framework callback with no test assembly on the stack. The new `ReportConfigurationOptions.SuiteName` overrides it for the cases the probe cannot know — one assembly sharded into several jobs that must stay distinguishable, or an assembly being renamed whose existing ids must survive. **An unresolvable suite reproduces the pre-3.1.0 id byte for byte**, so an ingest, a library caller and every report written before this release keep the ids they had rather than silently minting new ones. Scope, stated honestly: **145 of 905 ids collided across suites and 0 of 1,043 within any single report**, so this is a correctness fix for *combined* reports — a `merge` across suites, an ingest folding two runners, a cross-run ledger — and not a live bug in an ordinary run. The cost is real and is the reason it lands now rather than later: every `#sid-` deep link and every stored baseline written before 3.1.0 is keyed on the old id.
- **`statusCode` is a number, with `statusText` beside it.** It used to be one field holding the .NET enum *name* when .NET had one (`"OK"`, `"BadRequest"`) and the raw text when it did not (`"Ack"`, `"Responded"`) — so the field's type depended on whether .NET happened to name the code. That made `--status` half dead: measured against a report containing a `BadRequest`, **`--status 4xx` and `--status 400` each matched nothing** while `--status BadRequest` matched one, because the filter parsed an integer out of a string holding a name. The range form is advertised twice in the usage text and had **no test at all**, which is how it survived. Now an HTTP response carries `400` and `"BadRequest"`, a broker acknowledgement carries text and no number, and a tap that captured only a number gets the name back when .NET has one — so grepping for `NotFound` still finds a 404 whichever side it came from. All three filter forms work and are pinned. `kronikol query` reads **both** shapes, so every report written before 3.1.0 still answers; that is not optional, because the scanner called `GetString()` on the token and would otherwise have thrown `InvalidOperationException` — a crash, not a degraded answer — on the first report written with a number.
- **`Failures.jsonl` opens with a header line.** `{"formatVersion":1,"kind":"header","kronikolVersion":…,"suite":…,"scenarios":N,"failures":M}`, then one `"kind":"failure"` line per failure. It replaces a `formatVersion` repeated on every record, which stated the contract N times and **never once for a run that had no failures** — a green run wrote zero bytes, so "the file is empty" and "the file was never written" were the same bytes to a reader, and the absence of this file is precisely the signal that the run did not finish. The header also carries the denominator: a consumer reading only the failure lines cannot otherwise tell three of four from three of four thousand.
- **`ctrf-report.json`, and a `kronikol ctrf` verb to produce one from a report that has already been written.** Every other file Kronikol emits is read by a person or by `kronikol query`; this one is read by somebody else's action - a GitHub annotation step, a PR comment bot, a flaky-test dashboard - and those tools will never learn Kronikol's schema. One CTRF test per scenario carrying `status`, `duration`, `message`, `trace`, `filePath`, `line`, `tags`, `retries` and `flaky`, with `summary`, `tool` and, on CI, an `environment` block filled from `ciMetadata` (including `runAttempt` in `extra`, the only field that tells a re-run from the run it retried). Kronikol's five `ExecutionResult`s map onto CTRF's five statuses - `Skipped` and `SkippedAfterFailure` both become `skipped`, `Bypassed` becomes `other` - and the original word survives as `rawStatus`, so the translation loses nothing. `extra.kronikolAddress` carries the `sN` address, so a consumer that finds a failure can hand it straight back to `kronikol query`. **`specVersion` is pinned rather than tracked**, and the test that holds the writer to it lists every key the schema declares and fails on a sixth - a field emitted under a name CTRF does not know is silently dropped by every reader and reads as a Kronikol bug to whoever expected it. Deliberate omissions: per-test `start`/`stop` (the model has a duration and no start time, and a fabricated timeline is worse than an absent one), `attempts[]` (earlier attempts are folded before the report is built), and `filePath` where the lane supplied only a bare file name - CTRF consumers place annotations with it, so a name where a path is expected annotates the wrong file. The generated `retry N` labels are filtered out of `tags`, because `retries` already carries that fact and as tags they would grow a new one on every retry. `GenerateCtrfReport` is **off by default** - it is written for a consumer, and a run with no such consumer should not pay for it - and `kronikol ctrf <report> [--out <file>]` exists because by the time anyone wants CTRF the run is usually over: the report is a downloaded artifact, or came from a colleague, or belongs to a suite whose options nobody wants to change. Not a flag on `kronikol export` (that reads NDJSON captures and POSTs spans to a collector; every part of its contract is the wrong one here) and not a `query` verb (those answer under a byte budget, and a document that gets truncated is not a document). Both producers go through one writer, and a test asserts the converted document is byte-for-byte the one the run itself would have written.
- **`Specifications.md`** - the living documentation as prose, in a file small enough to read whole. The same question already had two answers and neither fits a reader who only wants the specification: `Specifications.html` is for a browser (one measured report was 10.7 MB, one embedded diagram 663 KB), and the YAML/JSON/XML trio answers a parser rather than a person. The markdown carries every feature, rule, scenario and step, and **carries two things the data trio drops entirely** - `Scenario.Rule`, which groups the scenarios written under it, and `Scenario.Description`, the author's own prose - because a narrative without them reads as an ungrouped list nobody wrote. Author prose is emitted as a block quote so a `##` inside somebody's Gherkin description cannot restructure the document around itself, and a step written across several lines stays one list item. No result, no duration, no interaction, no diagram: the same suite run red and run green produces the same bytes, which is what makes it safe to commit to a docs site. It is deliberately **not** blanked on a failed run the way `Specifications.html` and the data file are - those exist to stop a broken build publishing half-truths, but an agent reading this file is trying to understand the system it is debugging, and a blank file at exactly that moment is the least useful thing it could find. `GenerateSpecificationsMarkdown`, default off, named from `YamlSpecificationsFileName` so renaming the data file renames its narrative with it, and suppressed by `ExpectedTestCount` like every other specification output.
- **`kronikol.test.result` on every exported span, and `kronikol export --tests` to supply it.** The OTLP export could say which test a call belonged to and not how that test ended, so a trace in Tempo could not be filtered to the failures. The verdict is **not in the captured record and cannot be**: every field on a captured call is written at capture time, while the result exists only once a framework adapter has assembled the run — forty-odd tracker packages construct that record and none of them knows the outcome. So it arrives through a new `OtlpExportOptions.TestResult` lookup instead, keyed on the test id, which **is** the scenario id; a host holding the finished features wires it in one line. A delegate rather than a dictionary, so a caller with a large run need not materialise one. It is set in the one place every span is built, so batch, streaming and CLI all pick it up together. On the CLI, `--tests <file>` reads the run's companion tests NDJSON — the same file `kronikol ingest --tests` takes — and maps each `end` event through the same status table the ingest uses, so a span attribute and the report those files would produce never disagree; an unrecognised status word still maps to `Failed`, and the words are named on stderr rather than reddening spans in silence. A tests file inside an input directory is not also swept up as an interaction capture. **The streaming sink cannot carry the attribute** — it POSTs on a flush interval while the test is still running, so no verdict exists yet, and holding spans back until one did would break the rule the sink exists to keep, that capture never blocks. That limit is documented rather than worked around.
- **`ciMetadata.runAttempt`** — which try of a run the report came from, the eighth key of the run-identity block added earlier this release. **A provider's run id does not change when a run is re-run** (GitHub's own variables reference is explicit that `GITHUB_RUN_ID` "does not change if you re-run"), so a report that records the id and not the attempt folds a retry onto the run it retried — and re-running a failed job until it passes is exactly how a flake would disappear from the record. It is the one part of a run's identity that cannot be reconstructed afterwards, which is why it lands now rather than when something reads it. From `GITHUB_RUN_ATTEMPT`, counting from 1; null off GitHub Actions, because Azure DevOps exposes no equivalent variable and a null is honest where a guess is not. It rides every path the other seven do — JSON, XML, YAML, the JSON Schema and the XSD, the mergeable reader, and the query index — and `kronikol query summary` prints `attempt 2` on the `run:` line past the first attempt and carries `runId`/`runAttempt` in `run.ci` under `--json`, so it is readable rather than merely written.
- **`kronikol query --json`** — one envelope instead of text, on the seven verbs whose output is a list of like things: `summary`, `scenarios`, `failures`, `services`, `interactions`, `assertions` and `diff`. `{ formatVersion, command, report, kronikolVersion, notes[], items[], total, truncated, next }`, camelCase, one object on one line. **Text stays the default and is what to read in a terminal** — the same answer as JSON costs roughly twice the tokens — so this is for scripts and for the MCP wrapper that hands structure to a caller who never sees a terminal. Every other verb refuses `--json` with exit 2 rather than answering with an empty envelope: a step tree, a payload and a chronology are shaped for a reader, and an object form of them would be either an array of rendered strings or a second data model to keep in step with the first.

  The format lives in `QueryWriter`, which every verb already writes through and which is constructed in exactly one place, so a verb costs one projector lambda rather than a second rendering path — and no verb can page, truncate or warn in one format and not the other. What the envelope has to say that layout used to: `notes[]` carries every `!` caveat and every sentence that stands in for an empty result (`services` answering "no services were called" is a note beside an empty `items`, because absence is the answer that verb exists to give and an array cannot say it); `next` is the flags to append, already carrying `--json`, or `null`; `truncated` is a field rather than a line, and items are costed and appended whole so a cut-off listing is still one well-formed object. `--count` replaces `items`/`total` with `count`. `summary` and `diff` are not lists and say so with named members — `run`, `failed`, `slowest`, `diagnostics`; `left`, `right`, `tracking`. `failures` items reuse the field names the run already writes into `Failures.jsonl`, so one concept does not reach a consumer under two vocabularies. Errors are unchanged: plain text on stderr, nothing on stdout, exit 2 or 1.

  Three row builders that produced pre-rendered strings became records so their rows could be serialized at all — `interactions --group` (`CollapsedCall`), the body diff's twelve row sites (`DiffRow`) and the run diff's five sections (`RunChange`) — each rendering back to the identical text, which the existing text pins hold them to.
- **`--out FILE` works on every query verb, and lifts the byte budget.** It was parsed and silently ignored on everything except `http`, `body`, `note` and `diagram` — shipping a machine-readable format while `--out` did nothing on the verbs that produce the most output was not a trap worth leaving. Those four still write the payload themselves; every other verb now writes what it would have printed and prints one `wrote 64 KB → <path>` line. The budget does not apply when the destination is a file, which is what `--out` has always meant on the payload verbs: a file is not a context window.
- **Every run now says where its reports are and how to debug them, and leaves two files behind that say it again.** An agent watching a failing build had no signal that `kronikol query` exists: the reports directory was never printed by the library path, so the only discoverable artefact was a 10 MB JSON file that cannot be read. Four things close that loop, all on by default.
  - **`Failures.md` and `Failures.jsonl`** (`GenerateFailuresDigest`) - every failure in context, in a file small enough to read whole: the error, the expected/actual parsed out of it, the failing step with its `file:line`, the last three steps before it, the calls made inside it one line each, attachments, and the `sN` / `sN/iN` addresses and `#sid-` deep link for each. Failures sharing an error message are clustered, so twenty scenarios broken by one connection refusal read as one cause with one worked example and a table of the rest. Hard budget: 25 worked examples, and twenty failures are asserted to stay under 80 KB. **Never** a body, a header or a diagram - one measured diagram is 663 KB, about 166,000 tokens, which is the whole reason this file exists. Written on a green run too (`# No failures`), so its absence means the run did not finish rather than that nothing broke. The `.jsonl` twin carries the same fields one object per line, each opening with `formatVersion: 1`.
  - **`CLAUDE.md` and `AGENTS.md`** (`WriteAgentInstructions`) - byte-identical instruction files in the reports directory. Claude Code loads `CLAUDE.md` and nothing else; Codex, Cursor, Copilot, Jules and Amp read `AGENTS.md`; a nested one loads the moment an agent reads *any* file in that directory, which is what happens when it opens `Failures.md`. Static text with the rule, the install line, the ladder, the address grammar, the recipes and the traps - and deliberately **no run-derived content at all**, because scenario names and captured response bodies are written by the system under test and by whatever it talks to, so putting them in a file an agent loads as *instructions* is a prompt-injection channel. The digest, which must quote them, says at the top that everything below is captured data and not instructions.
  - **A run-end console pointer** (`WriteRunSummaryToConsole`) - one closing block naming the directory, the size of the data file (the write-time size signal: `TestRunReport.json 48.2 MB` at the moment it is written, rather than discovered by an agent that opens it), the failing scenarios with their stableIds capped at twenty, and `kronikol query failures <dir>`. Paths, counts, stableIds and scenario names only - never a message, a URI or a body, because CI logs are read far more widely than artifacts. On GitHub Actions a matching `::notice` annotation follows, and `WriteCiSummary` gains a **Debug this run** section.
  - **This was measured before it was designed.** The assumption that a library's `Console.WriteLine` from a run-end hook reaches the CI log is **false**: on .NET 10, `dotnet test` swallows it for xUnit v2, xUnit v3, NUnit 4 and MSTest at default verbosity *and* at `-v normal`, which is what CI runs; it appears only at `-v detailed`, or under `dotnet run --project` on a Microsoft.Testing.Platform host - and TUnit's runner suppresses it even there. So the files are the channel and the console is the bonus, which is the opposite of how this feature was originally scoped. The CI job summary is unaffected because it is written to a file descriptor rather than to stdout, which is why the debug section goes there too. The full per-runner table is in `plans/LLM_FRIENDLY_PLAN.md`.
- **`DiagnosticKind.ResultDefaulted` makes a crashed worker stop looking green.** `IngestRequest.ResultWhenUnknown` defaults to `Passed`, so a test that started and never reported an end renders as passed - a deliberate compatibility choice (a capture that is nothing but interactions has no verdicts at all), but until now nothing in the report distinguished that default from an observation. Ingest records how many scenarios took it and what value they took, the entry travels in the data file's `diagnostics`, every `kronikol query` command prints it before its answer, and `Failures.md` carries it as a warning **above `# No failures`** - which is the exact case where the reassurance is wrong. `FeatureSynthesizer.Result` gains `DefaultedResultCount`.
- **`#sid-<stableId>` deep links: the address an agent prints is now an address a human can open.** `Failures.md` has been emitting `TestRunReport.html#sid-…` since M1 into HTML that could not resolve it. The report's only anchor was `#scenario-<slug>` — a slug of the display name, so it moves the moment anyone renames a test, and two features naming a scenario the same thing get the same one. Every scenario `<details>` and every example row now carries `data-stable-id`, computed exactly as the data file computes it (feature name, display name, outline id **and** example values), so a link built from the JSON resolves in the browser; a contract test asserts the two sets of ids are equal. `parse_url_hash` learns the `sid-` form, and an anchor now rides in front of the filter grammar (`#sid-…&status=Failed`) instead of replacing it, so a link can carry both and `update_url_hash` keeps the anchor when a filter rewrites the rest — previously the first filter click destroyed it. Three smaller things fall out of the same walk:
  - **Clear All no longer throws the link away.** Clearing filters is not the same act as leaving the scenario somebody navigated to; the fragment survives, the filter state does not.
  - **A link pasted into an already-open report now works.** `parse_url_hash` only ever ran on `DOMContentLoaded`, so changing the hash on a live page did nothing at all — which is exactly what happens when a person is handed a link mid-session. A `hashchange` listener resolves the anchor (and only the anchor: `update_url_hash` rewrites through `replaceState`, which fires no event, so there is no loop).
  - **`#scenario-<slug>` pointing at an example row opens its group.** It found the `<tr>` by id, called `setAttribute('open')` on a table row — a no-op — and scrolled to something still inside a collapsed `<details>`. The anchor walk now opens *every* enclosing `<details>`, and because an outline row is rendered twice (the flat parameter table, displayed by default, and the grouped one behind it) the link resolves to whichever copy is actually on screen rather than selecting a row inside `display:none`.
  The link is emitted by `kronikol query failures` and `kronikol query steps` as well, but only when the HTML is sitting next to the data file — a link into a report that was never generated is worse than no link. `CLAUDE.md`/`AGENTS.md` gain the `#sid-` row in their address table: it is the one address that is for a person rather than for the tool. Same renderer builds both reports, so `Specifications.html` gets the attribute and the deep links too.
- **The templates ship the agent skill, and `kronikol init-agents` retrofits it.** The skill that teaches an agent to query a report instead of opening it existed, and the only way to get it was a wiki paragraph telling a human to copy three files out of GitHub — so almost nobody had it. Now all twelve `dotnet new kronikol-*` templates scaffold `.claude/skills/kronikol-test-debugging/` plus a `CLAUDE.md` and `AGENTS.md` carrying the instruction block, and the new **`kronikol init-agents [<dir>]`** installs the same five files into a repository that predates them. Both names are written because Claude Code reads one and Codex, Cursor and Copilot read the other; a run's own reports directory has done the same since earlier in this release. `init-agents` is **safe to re-run, and re-running it is the upgrade path**: the block lives between `<!-- kronikol:begin -->` and `<!-- kronikol:end -->` and is replaced in place, so a second run neither duplicates it nor disturbs a line of what you wrote around it, and an unchanged file is reported as `unchanged` rather than rewritten. It matches the line endings of the file it is appending to, keeps a byte-order mark the file already had, and refuses — changing nothing — a directory that does not exist, a file that is not valid UTF-8 (an older Windows repository's `CLAUDE.md` may be Windows-1252, and decoding it would destroy every non-ASCII byte outside the markers), a file that opens a block and never closes it, and a file that already contains the block twice. Markers count only on a line of their own, so a repository whose instructions *document* this feature — as the wiki, the README and the command's own help all do — is not mistaken for one that installed it. A file it cannot write is a message and exit 1, not a stack trace. There is exactly **one copy of these files on disk** — `templates/skills/` and `templates/agents/CLAUDE.md`; the twelve templates pack it by package path rather than by duplication, the tool embeds it, the Kronikol repository itself installs it, and a test pins all four to the same bytes. The packaging was verified rather than assumed: a dot-directory survives `dotnet pack` only because `NoDefaultExcludes` is set, and survives `dotnet new` only because the template engine's default excludes contain no dot-rule — both now checked in CI against the real package and the real scaffold, for all twelve templates.
- **`kronikol query grep --in` refuses a target it does not know.** An unrecognised target used to be dropped in silence, so `--in bodys` searched nothing, printed nothing, and read exactly like proof that the value is not in the report — a false negative on the single question `grep` exists to answer. It now exits 2 and lists the six valid targets. `--sort` gets the same treatment on both views that take it, including the case where the value is valid on the *other* one: `services --sort bytes` is real, `interactions --group-by ... --sort bytes` is not, and it used to fall through to the default ordering as though it had been honoured. Both values are lower-cased when parsed, because both validators are case-insensitive while the code that consumes the value is not — accepting `--in BODIES` and then searching nothing would have put the same silence back one capital letter further from view.
- **`kronikol merge` writes the merged data file, not just the merged HTML.** A sharded run produced one combined report an agent could look at and nothing it could query: to ask what failed you had to go back to the shards and ask each of them. Merge now writes the merged `TestRunReport.json` beside the HTML, in the same mergeable format the shards were in, so the combined run is **queryable** (`summary`, `failures`, `services`, `interactions`, `body`, `diff` — one file instead of every shard), **re-mergeable** (merging is closed under itself; yesterday's combined file is a valid input to tomorrow's merge) and **promotable** (copy it to `<reports>/baseline/` and the next run diffs against the whole suite). `--no-json` opts out. Two guards: the data file is written only **after** the HTML render succeeded, because a data file beside a report that was never written is worse than neither — the next command finds it and believes the merge worked; and it is never written over one of the inputs, since `kronikol merge ./artifacts -o ./artifacts/runner1.html` would otherwise overwrite the shard it just read and the next run of the same command would merge its own output back in. The format version does **not** move: the merged file carries the same keys a shard does, so an older `kronikol query` reads it without knowing anything changed. New public `MergeableReportRenderer.Serialize` and `MergeFiles`.
- **The mergeable format carries the captured traffic — it was calling itself a strict superset of the standard report while dropping the single largest thing the standard report holds.** `GenerateMergeableReportJson` passed `logLookup: null`, so every mergeable file ever written had zero `httpInteractions`, zero `annotations` and no `stepPath` attribution. The consequence was silent, because an empty result is indistinguishable from a run that made no calls: `services`, `interactions`, `http`, `body`, `values`, `flow`, `trace`, `annotations` and `grep --in bodies|uris|headers` all came back empty on a mergeable or merged file, and the wiki had promised the opposite since the feature shipped (`kronikol query interactions merged.json --group-by service,status` was documented as "the cross-runner comparison a merged file exists to enable"). Shards now write their traffic and the merge carries it through. Two things do not round-trip and so are carried explicitly rather than re-derived: **`stepPath` and the exported annotations**, because deriving them walks the diagram markers, and the markers are dropped at write time by design. The **run's `diagnostics`** now round-trip too — without them a merged report stopped warning that scenarios had been defaulted to `Passed`, which is precisely the warning that stops a crashed worker looking green.
- **`kronikol query diff <report> --baseline`.** An agent knows where the current report is; it should not also have to be told where last-green lives. `--baseline` names only the current report and resolves the other side: `<reports>/baseline/TestRunReport.json` beside it (`<reports>` taken from where the report actually resolved, not from the raw argument), else `$KRONIKOL_BASELINE` — a report or a directory holding one, resolved exactly as any report path is — else exit 2 naming both. **Convention wins over the variable**, so a stale exported `$KRONIKOL_BASELINE` cannot quietly override what is on disk. The argument order inverts deliberately (`diff old new` names the *old* report first; `--baseline` names the *current* one) and the orientation is settled before anything prints, because a diff running backwards reports every regression as a fix and still exits 0. Both files are usually called `TestRunReport.json`, so when the names collide the two header lines fall back to paths relative to their deepest shared directory. A `baseline` folder is invisible to every directory scan — `query <dir>` will not offer it as a candidate and `merge <dir>` will not sweep it in as a shard — so adopting the convention changes nothing else. The environment is read through an injected accessor, the repo's existing convention, so no test ever mutates process environment.
- **`kronikol query diff` reports tracking losses.** A **Tracking** section lists every service that captured *fewer* calls than in the older run, worst first, and marks any that fell to zero as `no longer tracked`. This is the regression no test failure catches: an upgrade or a configuration change stops a client being tracked, every test still passes, and the diagrams quietly lose a participant. Requests only, so a missed response does not read as a lost call; silent when the newer run captured *more*; and skipped entirely when either report has no interactions at all, since absent is not the same as lost. It exists because the wiki's "Gold Standard" tracking-fidelity check was a hand-rolled PowerShell script that loaded both reports with `ConvertFrom-Json` — exactly what `kronikol query` exists to avoid on a report that is tens of megabytes — and compared a single global total, so one service falling to zero while another grew by the same amount looked like no change at all. That workflow is now three lines and a `--baseline` diff.
- **A downloaded `TestRunReport.json` can now say which run it is.** `ciMetadata` (provider, build number, branch, commit SHA, pipeline URL, repository, run id) reached only the mergeable superset and the HTML summary table, so the standard data file — the one that travels in a CI artifact — could not say which commit it belonged to, which is exactly what a baseline comparison has to key on. It is emitted now, together with a new `environment { os, runtime }`, in JSON, XML and YAML, positioned between `endTime` and `features` so a streaming reader and a person running `head` both see it without the megabytes after it. Two decisions worth knowing:
  - **Unconditional.** Off CI the same seven keys come back with `provider: "None"` and nulls beneath them. A shape that appears only on CI is a shape no consumer can rely on.
  - **The detector still returns null off CI**, deliberately. The HTML summary's CI table is gated on exactly that, so the obvious shortcut — having the detector hand back a `None` record — would put a `CI (None)` table on every report generated on a laptop. The all-null object is built inside the data writers, the mergeable format emits the same shape, and the merge reader collapses a `None` provider back to null so merged reports keep the same gate.
  `environment` is two fields and no more: no machine name, no user name, no environment dump. `kronikol query summary` prints `run: main @9f3c1b2  acme/shop  GitHubActions #412` when there was a CI to read, and stays quiet when there was not.
- **Scenarios say where they are written.** `feature.sourceFile`, `scenario.sourceFile` and `scenario.sourceLine`: a step has carried `file:line` since 3.0.47, but the scenario itself never did, so an agent that had found the failing scenario still had to guess which file to open. Both Gherkin lanes had the data and dropped it.
  - **Cucumber Messages**: the document `uri` has been carried on every parsed scenario since that code was written and nothing ever read it; the scenario node never declared `location` at all, though every producer emits one. Paths are normalised to forward slashes — the golden fixture captured from playwright-bdd on Windows says `features\kronikol-demo.feature` while cucumber-js on Linux says `features/…`, and somebody globbing for the path should not have to know which machine ran the tests.
  - **ReqNRoll**: `FeatureInfo.FolderPath` turns out to be a dead end — verified against Reqnroll 3.3.4, it gives the folder (`"Features"`) and the file name is unreachable from it, because a feature's `Title` is not its file name. The path comes instead from the Gherkin document `ExamplesBlockResolver` already holds for the `Examples:` lookup, through the same reflection seam and the same contract that any surprise degrades silently to nothing.
  A scenario outline reports its **declaration** line, the same one for every row: `exampleValues` is what says which row, and a per-row line would make two scenarios that are the same code look like different code. This is deliberately **not** the same contract as a step's `sourceFile`, which stays a bare file name because it comes from `[CallerFilePath]` on the build machine; the schema descriptions say which is which. The location shows up in `kronikol query failures`, `kronikol query steps` and `Failures.md`. Unit-test adapters still supply nothing — the seams exist (xUnit v3, TUnit and MSTest all expose a compile-time path) but every one of them is the absolute build-machine path, which is not something to bake into a downloadable artifact.
- **`scenario.attempt`, and a `×N` marker for scenarios that share an identity.** The `retry N` label has always been in the HTML; it is a display artefact a filter can match, not something a consumer of the data file can read. The field says the same thing structurally and uses the **same 1-based numbering as the label** — Cucumber's own wire value is 0-based, and a field disagreeing with the label printed beside it would be worse than no field. `kronikol query scenarios` marks rows sharing a `stableId` with `×N` and a later attempt with `attempt N`, so the case that used to surface only as `diff` behaving oddly is visible in the listing.
- **The report says when SQL parameters were not captured.** A failing statement that reads `WHERE id = @p0` and never says what `@p0` was is the one dead end a report cannot answer its way out of — the values are not in the file and no further query will find them. `Failures.md` and `kronikol query failures` now say so once. The wording matters more than the detection: **`LogParameters` alone changes nothing**, because the `-- Parameters:` block is appended only at `Raw` verbosity and the default is `Detailed`, and the EF Core interceptor captures no parameters at all while stamping the same `SQL` dependency category — so the message names both settings and says where it does not apply. Detection is scoped to a closed list of SQL-shaped categories: a `?` in a query string, a `:` in `host:8080` and an `@` in an email literal would each trip a placeholder scan run over everything. On the query side it costs a payload read, so it is bounded — failing scenarios only, SQL categories only, statements under 8 KB, stopping at the first hit; a green run opens nothing. Never on the console pointer or the CI annotation, which carry paths, counts, stableIds and scenario names only.
- `kronikol ingest` now ends with the failing scenarios and the `kronikol query failures` line, printed through its own writer (the library's pointer is suppressed for that command so it speaks with one voice). `.jsonl` joins the CI artifact upload allowlist.

### Changed
- **`kronikol query services` pages like every other listing.** It hand-rolled its loop and its footer, which made it the one listing that could overflow the byte budget with nothing said about how to resume. It goes through the shared pager now, so it honours `--offset`/`--limit`, announces truncation and hands back a `next:`. Its closing line — `N services · a service missing here was never called`, which says something a row count cannot — is preserved. A report with more than 200 distinct services now pages instead of printing them all; the footer says so.
- **The body diff's line-by-line fallback counts one row per difference, not two.** It added a `- old` row and a `+ new` row per differing line, so a diff of twenty lines reported `paths differ: 1-40 of 40`. The rendered text is unchanged — still two lines — but the count, the range and the offsets now describe differences rather than printed lines.
- **The CLI's command list lives in one place.** `Program.cs` held three hand-maintained copies — the blurbs printed with no arguments, the dispatch switch, and the sequence of `PrintUsage` calls behind `--help` — inside top-level statements that no test could reach, so a command could be routable and entirely absent from the help with nothing to say so. All three now read one table, and `CommandTableTests` asserts every command appears in every view, answers its own `--help`, and that `Program.cs` names none of them.
- **The schema is a contract in fact, not only in name** — every property carries a `description` (39 of 83 did; the undescribed included `result`, `errorMessage`, every core step field and most of `httpInteraction`), `stableId`, `stepPath` and `activityTraceId` carry `examples`, and a top-level `$comment` names the size trap and `kronikol query` — the schema is what an agent should read *instead of* the file, so it says so. Guarded forever by `Every_schema_property_has_a_description` next to the drift walker. The report JSON's bytes are unchanged; only the schema file differs. Kronikol4J pins this schema byte-for-byte (`ReportDataSchema.java` against a `.NET`-captured golden) — ledger entry in its README, port pending.

### Tests
- **`RunEndPointerTests` asserted Windows-shaped paths on a Linux runner.** The class pins how a path is *shaped* on the way to the console — that the reports directory travels through verbatim, and that the instruction file beside it is joined with the platform's own separator — and it did that against a hard-coded `C:\proj\Reports\CLAUDE.md` while the writer uses `Path.Combine`. Green on Windows, red on every CI runner, which is why the `Release` workflow failed at the `v3.1.0` tag and published nothing, and why `Core Tests` had been red since the run that introduced it. The fixture directory is now native to whichever platform is running and the expectation is built the same way the writer builds it.
- **A run that fails *inside a step*, at last — `Example.Api.Tests.CiPreview.FailingWithSteps`.** The digest emits its `calls` table only when a failing step carries an attributed interaction, and nothing in the repository had ever produced one: across 37 captured reports and **4,944 step objects, `step.status` was `Passed` 4,944 times**, no step anywhere carried a `failureMessage`, and every report with failures was step-less while every report with steps was green. So `calls` was `[]` on every row on disk and the feature was unverifiable from captured output. Six scenarios cover the shapes that have to be told apart: a failing step that made the call itself (the only arrangement that can populate the table), a second with the same message so clustering has something real to cluster, a third with a different message so the run has more than one cluster, a failing step that made no call at all, a failure inside a nested composite step, and one that passes so the digest has a denominator. It is deliberately red and therefore runs in `ci-summary-preview.yml` beside the other CI-preview fixtures, not in the merge gate. Two things it settled immediately: adapters **do** record a failed step, and the nested case **does** populate, because LightBDD marks the enclosing composite failed too.
- **The first mergeable shards ever written to disk.** `GenerateMergeableData` was off in every project in the tree, so `grep -rl mergeableFormatVersion --include=*.json` returned nothing and every assertion about `kronikol merge` rested on hand-built JSON. It is now on in two CI-preview projects — one green, one red — and their reports are committed as `tests/Kronikol.Tests/TestData/Reports/*.mergeable.json` so a test can read a real shard without running an example project.
- **`ProjectAssetTrackingTests`** — every file a project names by literal path must be *tracked by git*, not merely present on the machine that wrote it. `Kronikol.Tool.csproj` embeds `..\..\templates\agents\CLAUDE.md` by literal path, and while that directory was untracked the build was green locally and **CS1566 on a fresh clone**, which is what CI is: a hard compile error before a single test runs, invisible to every check that ran where the file existed. `File.Exists` is green on exactly the machine where the bug is invisible, so the question is put to git instead. Wildcards are held to a weaker rule deliberately — an `Include` matching nothing is not a compile error, it silently packs nothing — so they must match at least one tracked file. Mutation-proved: renaming `templates/agents/CLAUDE.md` reddens it and names the file.
- **`CommandTableTests.No_two_commands_share_an_implementation`** — two table entries pointing at the same command class is the one mis-wiring every other fact in that file is blind to. The first version of it passed **vacuously**: the entries are lambdas, so their own `Method` is a compiler-generated name that is distinct whatever it calls, and the IL walk that resolves through them aborted on one entry and silently fell back to that name. It now asserts that each `Run` resolves to `Kronikol.Tool.<Name>Command.Run`, which makes the vacuous pass impossible.
- **`--status` had no test at all**, which is how its range form stayed dead code. Pinned now as a `Theory` over `5xx`, `500` and `InternalServerError` — all three forms must find the same call — with a negative control (`3xx`, absent from the fixture) so a filter that silently matched everything would fail too.
- `ReportContractShapeTests` pins the three shapes an outsider parses across all three writers together, because a version key present in JSON and missing from YAML is the same defect as no version key at all for a consumer reading YAML.
- **The CTRF writer is pinned by a key list, not by a validator.** `CtrfReportGeneratorTests` walks the whole document and fails on any key outside a hand-transcribed set - the repo's own convention (`TestRunReportSchemaContractTests`) rather than a new package dependency - plus the five status mappings as a `Theory`, the millisecond unit, the epoch-at-second-precision run window, the retry and flaky arithmetic, and the address ordering under the `Order API`/`Order api` pair that separates the two comparers. `CtrfCommandTests` adds the one that matters most: **the document the verb converts and the document the run would have written are the same bytes**. Two writers for one format is how a format quietly forks, and that fact was proved load-bearing by mutation - dropping the feature-level `filePath` fallback and the retry-label filter from the CLI half turned it red, and nothing else did.
- The digest comparer fix is pinned twice: a unit fact over `Alpha`/`beta`/`Gamma` (which a full-pipeline run cannot reach, because `CapitaliseTitles` upper-cases every initial), and an end-to-end fact over `Order API`/`Order api`, which survives capitalisation and cross-checks the digest's address against the real `TestRunReport.json` ordering rather than against a hard-coded expectation.
- `SpecificationsMarkdownGeneratorTests` covers the narrative shape, the rule grouping, background steps kept separate, a multi-line step staying one list item, and the two facts that define what the file is: the same features run red and run green produce identical bytes, and a failed run still writes the whole narrative.
- Nine new facts for the verdict: four on the mapper (present when the lookup answers, absent by default, absent for an id the lookup does not know, and the same on both halves of a pair, since an unpaired response must answer as its request would) and five on the CLI, including a theory over four runner vocabularies and the case where the tests file sits inside the input directory. The two existing tests that would have covered this were **silently weakened** — both assert attributes by name and never the count, so a new attribute passed them unnoticed; each now has an absent-by-default twin. The mapper fact also pins the attribute's position, because the encoder emits in list order and the wiki table documents that order.
- Six new facts. `QueryJsonTests` gains four for the resume pointer: `next` is an array and a value with a space stays one element; the vector runs verbatim to the end of a filtered listing; it keeps the positional address it was scoped to (proved by resuming and asserting every row still belongs to that scenario); and the text twin quotes what it must. Each was mutation-proved — flattening the vector on spaces, and disabling the quoting — and the address fact caught a real slip while being written, an off-by-one that dropped the very addresses it exists to carry. `QueryCommandTests` gains the error-classifier pair over a new fixture of Kronikol's own non-HTTP status labels, success and refusal side by side.
- **`QueryStreamingTests`** — the claim the whole tool rests on, and the one no other test could fail. Every existing fact about `query` would still pass if `ReportScanner` called `File.ReadAllText`: the answers would be identical, right up to the real 10 MB report the plan is about. So these measure what producing the answer costs rather than what it says — allocation on the scanning thread, because four thousand tests share this process and working set says more about the neighbours than about the scan. Three run always: a 6 MB body-dominated fixture must scan for under a third of its size; holding the interaction count still while multiplying payload bytes 12x must not move the cost; and the index's body entries must be offsets with the payload still reachable through them. The fourth is REPORT_QUERY_PLAN section 3.6's >100 MB case against the gitignored `tools/query-bench` corpus, skipping with the one command that generates it. It found the 324 MB defect above on its first run.
- **`QueryJsonTests`** — 33 facts, in their own file so a change that forks the two formats fails somewhere that names which one broke. The envelope parses as one object on every supported verb; no verb falls back to `QueryWriter`'s wrap-the-rendered-text path (which exists to keep an unconverted verb honest, and must never ship); `kronikolVersion` is present-and-null rather than absent, so a consumer never branches on key presence; following `next` to the end terminates and yields every row exactly once — written as the loop a consumer would actually write, and the direct guard against the resume bug above. **Six of them re-assert the no-payload invariant in JSON**, because a projector is exactly how it would be broken by accident: serialize an `InteractionEntry` wholesale and a captured body ships inside `items[]` with nothing red, and the text facts that guard the same thing cannot see that path at all. The budget loop has a `--json` twin for the same reason.
- Five new paging facts in `QueryCommandTests`, and each was proved to catch its bug by re-applying the pre-fix arithmetic and watching it go red.
- The export fix is covered by a new `ExportFilteredHtmlRenderingTests`, which opens the downloaded file in a browser and waits for an `<svg>` to appear, asserts `#puml-data` rides along, and asserts no diagram container in the export is left unable to resolve a source. All three fail against the old export. Nothing existing could have caught it: the one export fact asserted a seeded script string appears in the exported text, which it always did.
- `QueryCommandTests` gains the duplicate-`stableId` run diff (broken and gone), the rejected `--raw` flag and the mergeable-format banner. New `TestRunReportSchemaContractTests` (walker, descriptions, `$comment`, examples) and `ReportGeneratorDiagnosticsScopeTests` (a normal run records a render failure in the JSON and the HTML; a host's scope is reused and sees the failure; two generations do not share a collector).
- The discovery loop is pinned at three levels. Pure-function facts: `RunEndPointerTests` (the format, the twenty-line cap, the one-line green shape, the GitHub notice, and a fixture carrying a secret-bearing URI asserted absent from every line), `FailuresDigestGeneratorTests` (addressing that matches `kronikol query` - including the case where feature ordering moves the failing scenario from `s0` to `s2` - clustering, the budget, one-line calls, no `@startuml` anywhere, redaction passing through, and every jsonl line opening with `formatVersion`), and `AgentInstructionsGeneratorTests` (both files identical, the rule and the install line present, a renamed report honoured, and a reflection fact that no `Build` overload can take run data). Integration: `ReportGeneratorAgentOutputsTests` proves the four files ship by default, each can be turned off alone, the digest's address resolves to the same scenario in `TestRunReport.json`, the pointer prints and can be silenced, the CI summary carries the debug section - and that a digest which cannot be written (a directory in its place) costs the digest and nothing else, with the failure announced. End to end: `ResultDefaultedDiagnosticTests` drives a worker that starts two tests and dies, then asserts the entry, the data file, three query commands' banners and the digest warning.
- The skill is pinned to the tool it documents by `SkillDriftTests`, and the matching is deliberately strict: `body`, `diagram`, `steps`, `flow`, `values`, `trace`, `note`, `grep` and `summary` are all ordinary English words in these documents, so a bare `Assert.Contains` would have passed on day one and proved nothing — the same bug class as the report-HTML substring assertions fixed in 3.0.82. A verb counts only where it is *demonstrated as a command*, inside a code span or fence, as a whole word not preceded by a hyphen. Written that way it started red on `body` and `diagram`, which SKILL.md had only ever used as nouns. The other facts: every verb has a reference entry and every documented verb is real, every documented flag is known to the parser and every parsed flag is documented, the `--group-by` dimensions and `--in` targets in the help and the reference match the arrays the tool validates against, every provenance banner is explained in the skill, all four copies of the skill are byte-identical with matching file *sets*, every template folder is in the pack list, and no shipped agent file contains one of the five strings `template.json` rewrites. `InitAgentsCommandTests` covers the install, both re-run paths, appending to a file that has content, replacing the block in place, CRLF, a missing directory and the argument errors. `FallbackScriptTests` runs `scripts/query.py` under the machine's Python (skipping where there is none — the probe tries both `python` and `python3`, because Windows has the first and the CI runner has the second) and makes its counts and its addresses agree with the real tool's.
- The review that found the paging and marker defects above was itself adversarial, and three of its findings were about the M2.8 tests rather than the code: the banner fact asserted three hand-typed strings and so could never fail in the direction that matters; the dimension and target coverage facts used bare `Contains`, which five of nine dimensions and four of six targets satisfied from unrelated prose (the 3.0.82 bug class, committed inside the test whose own docstring cites it); and `CommandTableTests` could not see a `query` entry mis-wired to `MergeCommand`, because merge's usage text happens to contain the words "kronikol query". All three are fixed: the banner list is now extracted from every `Line("! …")` site in the tool's source and each must be explained in SKILL.md, the coverage facts are anchored on the line that does the enumerating, and the help fact reads only the first line of the usage block. **The last was verified by re-applying the mutation** — it now goes red, where before it passed with the whole suite green.
- The merge output and the baseline diff are pinned in two new files. `MergedJsonOutputTests` covers the data file's existence and naming, `--no-json`, that it is the mergeable superset, that `query` answers over it, that merging is closed under itself, that the diagnostics and the producing run's `kronikolVersion` survive, that a shard carries its traffic and a merged file answers `services` and `body`, that a merged report no longer claims a scenario touched nothing, that a `baseline` folder is not swept in as a shard, that the write never lands on an input, and that a failed merge leaves no data file behind. `BaselineDiffTests` covers the full resolution order (folder, variable, variable-naming-a-directory, folder-beats-variable, the error naming both), the tracking section (a service lost, a partial drop, no warning when more was captured, and a report with no interactions not read as a total loss), and the regression guards: `diff old new` unchanged, a positional report still beating the convention, and a `baseline` folder not turning a directory lookup into an ambiguity. **The direction was verified by breaking it** — inverting the swap turns 7 of the 11 red, including the two that would otherwise report a regression as a fix.
- The batched JSON fields are pinned end to end. `RunIdentityDataTests` asserts the identical key set on and off CI, the position before `features`, the XML and YAML blocks, that no machine name or user name reaches the file, that a local report grows no `CI (None)` table, and that the detector's null contract is intact. `SourceLocationTests` walks a located scenario through all three writers, the merge round trip (including the step detail the reader used to drop), a two-shard merge where only one shard knows the path, `query failures` and the digest. `CucumberFeatureSynthesizerTests` gains the golden-fixture facts — the real uri, the real line, backslashes normalised, and every row of an outline pointing at one declaration. `ExamplesBlockResolverTests` gains the ReqNRoll source facts plus the degradation contract, and its reflection drift test now covers `GherkinDocument.Uri` and `Scenario.Location.Line` so a Reqnroll rename is loud rather than silent. `ParameterCaptureHintTests` pins the four placeholder shapes, the captured-parameters marker, the four false positives the scoping exists to prevent (`?` in a query string, `:` in a host:port, `@` in an email, a JSON body), that the message names both settings, that the digest says it once however many statements there are, and that a green `failures` run reads no payloads.
- The deep links are pinned at both levels. `StableIdDeepLinkTests` (unit) asserts the attribute on the emitted `<details>` and `<tr>` in context, that both copies of a flat-view outline row carry it, and — the fact that makes the feature work at all — that the set of ids in the HTML equals the set in `TestRunReport.json`. `StableIdDeepLinkTests` (E2E) drives the browser: a `sid-` link opens its scenario and leaves the same-named scenario in the other feature shut, a filter click keeps the fragment, a combined link applies both halves, Clear All keeps the anchor, a hash set on a live page resolves, and a row link lands on the displayed copy rather than the hidden one. The last two were verified red against the unfixed scripts.
- Three existing facts that had quietly stopped guarding anything are anchored properly rather than left green. `Deep_link_JS_handles_row_inside_parameterized_group` and `Report_expands_scenario_from_hash_on_load` were bare `Contains` calls over the whole report (`"data-scenario-id"`, `"location.hash"`) that would survive deleting the code they name — they now extract the function body by brace matching and assert inside it. `Flat_table_rows_have_no_id_attribute` guarded only the string `id="scenario-`, which a new id-shaped data attribute would not have tripped; it now asserts on ` id="`. Its flat-table slice — and the equivalent in the new test — started at the first mention of `param-table-flat`, which is in a script, not the table; both anchor on the emitted `<table>` now. Same bug class as 3.0.82.
- `FailureClusterReportTests`' two element-id regexes matched `[^>]*\bid="` greedily, and `data-stable-id="` ends in a word-boundary `id="` — the last match in the tag wins, so the pins would have captured a stable id as the element id. Tightened to `\sid="`, which cannot be reached by an attribute suffix.
- One design limit is now written down in a test rather than only in a comment: an `OutputFailure` cannot reach the data files' `diagnostics` array, because that snapshot is taken before the outputs run so the HTML and the JSON agree with each other - an output's own failure is necessarily later than the bytes it would have to appear in.

### Documentation
- `Generated-Reports.md`'s output table gains `ctrf-report.json` and `Specifications.md` - and, in the same edit, the four M1 files (`Failures.md`, `Failures.jsonl`, `CLAUDE.md`, `AGENTS.md`) that had never been added to it. New `## Specifications.md` and `## ctrf-report.json` sections, the first stating in as many words that it does **not** follow the blank-on-failure rule its two siblings do.
- `Querying-Reports.md` documents the `kronikol ctrf` verb and its exit codes; `Exporting-to-OpenTelemetry.md` says in one line that CTRF is a different verb, so nobody looks for it there. `Report-Configuration.md` gains both new option rows. `CI-Artifact-Upload.md` and `Capture-Time-Redaction.md` name the two new files, which are swept up by the upload allowlist and are in the same exposure class as `TestRunReport.json` respectively.
- `Querying-Reports.md` gains a `--json` section: the envelope key by key, a table of every verb's item shape and its extra members, and the three ways a page can end. Its shared-flag table gains `--json` and `--out`'s widened meaning, § Exit codes says plainly that `--json` does not change the error channel, and § Using it from an AI agent says which reader each format is for. The skill's `references/commands.md` carries the same envelope for a script author, and SKILL.md tells the agent reading a terminal **not** to use `--json`.
- `Generated-Reports.md` documents the two export buttons for the first time, including what the HTML export deliberately leaves out (toolbar, timeline, component diagram) and the blank-diagram defect fixed here.
- `CI-Artifact-Upload.md`: the upload allowlist has covered `.json` and `.xml` since the data export landed but the page still said `.html/.yml/.md` — corrected, with the consequence spelled out: `kronikol query` can be pointed at a downloaded artifact folder. `Querying-Reports.md`: the mergeable-format wording and the repeated-`stableId` rule of `diff`.
- The skill's own content caught up with what the tool grew in M0/M1: `Failures.md` is documented as rung zero of the ladder, the two provenance banners added this release are explained (including that a green run reporting `recorded no result` is not evidence anything passed), `body` and `diagram` have recipe rows, and there is a worked three-command investigation using real output from `examples/Example.Api/tests/Example.Api.Tests.CiPreview.Mixed`. `scripts/query.py`'s docstring said "the four commands that matter most" and then named five, of the six it registers; it now names all six, and its `steps` output leads with the scenario address and the `#sid-` deep link the way the real tool does, so an address the fallback prints can be pasted into the tool.
- `Querying-Reports.md` § Using it from an AI agent is rewritten around the two installs; `Project-Templates.md` gains the scaffolded agent files in "What's Included"; `AI-Integration-Prompt.md` replaces both copy-the-folder-by-hand instructions with `init-agents`; `templates/README.md` gains a "What's Included" section; `README.md`, `nuget-readme.md` (packed into every package) and the tool's NuGet description all name the new command.
- New `plans/LLM_FRIENDLY_PLAN.md` — the plan for making Kronikol the tool an agent debugs with. This release is its M0 (ride-along fixes) and M1 (the discovery loop); its §11 records what shipped, the decisions taken during implementation and the measured per-runner console table, and a `plans/PLANS_STATUS.md` row tracks it.


## [3.0.86] - 2026-09-12

### Fixed
- **Copying a note handed back Kronikol's own line breaks** (user-reported) — the width budget writes real line breaks into a note body so a diagram stays under its limit, but every path that hands note text back to a reader read those source lines and joined them with `\n`. When the break had landed mid-token, which is the common case, the token came back in two pieces: a JWT copied out of a report did not parse. **Patch bump** — a bug fix; no new public surface, though generated diagram bytes change. Six sites insert breaks: the 120-character unbreakable-run wrapper used by payload notes and (since 3.0.85) user-action note bodies, the 80-character chunkers for full paths, form-url-encoded fields and gray header values, and (since 3.0.85) the 110-character assertion-note wrapper. Each now marks the break it writes with an invisible `<U+200B>`, and one shared rejoin undoes exactly those. The escape form is deliberate rather than a literal character: `EscapeCreoleMarkup` escapes `<`, so a payload that literally contains `<U+200B>` arrives as `~<U+200B>` and an **unescaped** marker is provably Kronikol's own — the same argument the YAML reconstructor already makes about focus-emphasis tags. Measured on real Java PlantUML before implementing: a marked note and an unmarked one draw at **668x145 either way**, so the marker costs nothing, and a Playwright fact pins that the browser engine resolves it the same way. **Copy box text**, **Open box text in new tab** (both variants of each), **Copy all caller request payloads** and **Copy Highlighted Text** all go through it now.
- **Two breaks need two markers.** The assertion-note wrapper breaks between words, where it *consumes* the space that was there, and inside a word, where it does not. A single marker reassembles one of them wrong, so there are two (`<U+200B>` and `<U+200B><U+200B>`).
- **A long padded line lost its column alignment before any break was inserted** — `Atoms` split with `RemoveEmptyEntries`, collapsing every run of spaces to one, so a padded assertion message over 110 characters came out mis-aligned and no rejoin could recover it. That silently defeated the monospace note control shipped in 3.0.85. Runs of spaces are preserved now (`WrapOneLine` tracks whether a line has started rather than testing `current.Length > 0`, since an empty atom is a legitimate atom and a leading one means the line began with a space).
- **`Copy all caller request payloads` leaked creole escapes** — it neither unescaped nor rejoined, so payloads came back carrying `~` before every `/`, `*`, `_`, `"`, `[`, `<`, `#` and `=`. It shares the one copy path now.

### Changed
- **Deep search rejoins on the marker instead of guessing** — normalization rule 5b welded together any note-body line that did not start with whitespace, on the assumption that a flush-left continuation could only be a wrap. Right for indented JSON, wrong for a plain-text or SQL payload whose real lines start at column 0, which lost its line breaks in the index. With every break marked the guess is unnecessary: the rejoin moves to pass **1b**, ahead of the ASCII fold and the creole-escape strip (pass 3 deletes the `~` the guard depends on), and rule 5b is removed along with the note-opener matcher, the JS-whitespace class and the trims it needed — 93 lines. Search results change for notes holding multi-line flush-left payloads; the shared cross-language vectors move with it, and `SearchNormalizer.cs` now calls the generator's own rejoin rather than restating it.

### Tests
- `NoteCopyFidelityTests` (unit) pins the marker at all six sites, both break kinds, the escaped-marker guard, space-run preservation and the CRLF contract; `NoteCopyFidelityScriptTests` pins the client wiring, including that the rejoin runs **before** the creole unescape (the reverse order turns a payload's `~<U+200B>` into a bare marker and eats a real newline). Two IKVM facts pin through real Java PlantUML that a marked note draws at the same size as an unmarked one and still parses. Playwright `NoteCopyFidelityTests` reads the real clipboard from a report whose note was wrapped by the real generator: the token comes back whole from Copy box text, Open box text in new tab and Copy all caller request payloads, and the marker never shows in the painted diagram. Every one fails against a build without the marker.
- Four existing facts that measured a wrapped line's bytes were re-pinned, each made stronger: they used to delete newlines and compare, and now put the text back through `RejoinMarkedLines`, which is the contract the marker exists for.
- **A defect found by the E2E suite, not by review:** the deep-search Web Worker is built by serialising a fixed list of functions with `toString()`, so the two new rule-1b helpers did not exist inside it — the first normalization threw and deep search never returned, no error, just a timeout, across all 18 `DeepSearchTests`. The roster carries them now, their marker constants are literals inside the function bodies rather than module-scope `var`s, and a structural test guards both.

### Documentation
- `plans/NOTE_COPY_FIDELITY_PLAN.md` is committed as the design record, with the measured before/after and the four things the execution changed about the plan. Wiki `PlantUML-Browser-Rendering` gains a "Copying note text" section and `Deep-Search` records the rule 5b -> 1b change. Kronikol4J: the divergence ledger gains 3.0.86 (marker emission at all six sites, plus the normalization change its search port would need).

## [3.0.85] - 2026-09-11

### Added
- **Notes can be read in a monospace font, and widened to fill the diagram** (user-requested: *"we want things like sql queries easy to read"*; `BrowserJs` reports). Two glyphs join the `Y`/`J` format toggle in a note's hover cluster — **`M`**/**`A`** switches that note's text between the engine's proportional font and `Courier New`, **`↔`**/**`→←`** widens it to fill the diagram's container and back — with compact **Aa / Mono** and **Fit / Full** dropdowns beside the filter toggles applying either to every note at once, report-wide or per scenario, and `TestRunReportToggleDefaults.NoteFont` / `.NoteWidth` starting a whole report that way.

  **Monospace is the larger half of this, and it is the half width cannot substitute for.** PlantUML draws note text in a proportional font, and the payloads that most need reading are written in columns: analytics SQL pads its `AS` clauses, nested `CASE`/`WHEN` blocks are indented, tabular text is aligned. Measured on five note lines whose `AS` token sits in the same source column, the proportional font scattered them across **65.8 pixels** and monospace put every one of them at the same x. Kronikol names **`Courier New`** rather than the generic `monospace` for a measured reason: the engine sizes the note box from its own metrics and the browser paints the text into it, and the two resolve font names independently — across seven candidates it is the only name both resolve (`monospace` sizes the box with a generic mono metric and the browser paints *sans* into it; a name neither resolves is not a no-op either, it sizes the box with a third metric again, and `document.fonts.check()` is useless as a guard — it returned `true` for a font that does not exist).

  "Full width" means **fill the container exactly**, not "as wide as possible": `svg { max-width: 100% }` scales a diagram wider than its container *down* rather than scrolling it, so past the container wider text is smaller text. The target is computed from the drawn diagram's slack, with one correction pass after the re-render — widening a left-anchored request note pushes the participants right rather than extending the canvas, so the relation is not quite linear. The `↔` glyph is offered only where widening would change something (painted rows against source lines), and a widened note keeps it so there is a way back.

  Both ride the same mechanism, which is the **only** per-note width handle PlantUML has: a `.className { … }` style block plus a stereotype on the note header. Measured on the shipped engine and on real Java PlantUML, the element selectors — `note { … }`, `note<<x>> { … }`, `sequenceDiagram { note { … } }` — all silently ignore `MaximumWidth`; only the class selector carries it. Nothing is emitted at generation time: the class and the style block exist only in the client-side re-render source, the way the YAML view's spliced lines do, so a report with nothing toggled is byte-identical. The four note-header matchers that assumed at most one stereotype were widened together, because a skew between note blocks and rendered note groups is exactly what the note-button index tests exist to catch.
- **`ReportConfigurationOptions.DiagramNoteWrapWidth`** (`int`, default `800`) — how wide a note body is drawn before PlantUML breaks it at a space. This is the only note-width control that reaches `Server`, `Local`, `NodeJs` and a source copied out of the report, because the two controls above are client-side. Valid range **720–4096**: the floor is the width a form-url-encoded note chunk needs to stay on one drawn line (below it the engine breaks a chunk mid-line and can cut one of Kronikol's own inline colour tags in half), the ceiling is PlantUML's own `PLANTUML_LIMIT_SIZE`. A value outside it fails generation with the range in the message.

### Fixed
- **Diagrams are no longer drawn thousands of pixels wide** (`plans/DIAGRAM_WIDTH_PLAN.md`, executed). Length caps and height splitting left one axis unbounded — how *wide* a diagram draws — and it costs twice: a diagram wider than its container is scaled **down**, not scrolled, so past the container more pixels mean smaller text; and PlantUML's default `PLANTUML_LIMIT_SIZE` (4096) does not reject an over-wide diagram, it **crops** it, silently, keeping the top-left corner with no error anywhere. Every measurement below is the drawn size through **real** Java PlantUML, because the source is always valid and asserting on it cannot see this class of defect at all.

  | Emitter | Was | Now |
  |---|---|---|
  | sequence participant names | 5,650 px for an 800-character name | broken onto 80-character lines |
  | user-action message labels (a Playwright locator chain) | 11,154 px | broken onto 100-character lines, before the statement cap |
  | user-action note bodies | 13,165 px — and they bypassed the note formatter, so a `**` in a detail string styled the text instead of showing | creole-neutralised and bounded like every other note body |
  | assertion notes | 8,003 px for a connection string | broken onto 110-character lines |
  | step-delimiter bars | 9,450 px for a step quoting a JWT | broken onto 110-character lines |
  | step-bar table cells | 5,243 px for one payload cell — creole cells never wrap, not even at spaces | elided against a shared row budget |
  | internal-flow activity diagrams | 11,032 px for one EF-shaped `DisplayName` — the only family with neither a cap nor `wrapWidth` | both |
  | diagram titles | 7,978 px — titles never wrap at any width | broken onto 100-character lines |

  `skinparam wrapWidth` could not have fixed any of them: measured against real Java PlantUML it does not reach message labels, `loop`/`group` labels, participant boxes, titles or activity diagrams at all, and where it does apply it only breaks at whitespace — and the text that gets long (locator chains, connection strings, tokens, fully-qualified type names) has none. The breaks are in the source, from one shared width-budget wrapper the component diagram's 3.0.83 rules now also use.
- **A test that touches many services no longer draws one diagram wider than the screen.** Sequence width accumulates about **78 px per participant** even with names as short as `Service37` — 50 services drew 4,035 px and 60 drew 4,817 px, in *one* diagram, because encoded length and estimated height were the only split triggers and a test that is merely wide reaches neither. A diagram now also splits when its participant row would outgrow the budget. Splitting alone was no bound either: the prefix is built from the whole test's traces, so every fragment re-declared every participant the test ever touched and came out as wide as the unsplit diagram — each fragment now declares only the participants it draws (single-fragment diagrams, which is nearly all of them, are unchanged byte for byte). Unlike the other two triggers this one applies under `BrowserJs` as well, because participant count, unlike length and height, does not change with note state.
- **A multi-line assertion failure message is drawn as multiple lines.** The ingest-side emitter folded real newlines into literal `\n` escapes, and measured on real PlantUML a `\n` inside a `note … end note` block draws as the two characters and breaks nothing (660 characters on one line drew 4,341 px; the same text carrying five `\n` escapes drew 4,395 — *wider*, not narrower; only real newlines brought it to 766). A diff in an assertion message therefore came out as one very long line with literal `\n` in it.
- **A step whose text has no spaces in it is now readable.** A step quoting a JWT, a base64 blob or a GUID list was truncated at ~1,400 characters (past which the coloured bar overflows the engine's own JavaScript stack) and what survived was still one line about 9,450 px wide. Such a step now switches to the styled `<<stepBody>>` form, which carries no inline colour tag and so rides the far higher note-line ceiling, and its text is broken onto display lines. A step that already fits keeps the legacy coloured bar byte for byte.

### Changed
- **Report bytes change for any suite with a long participant name, a long UI action label, a wide assertion note, a wide step bar or an internal-flow activity diagram**, and every `BrowserJs` report gains the two note-appearance dropdowns in its toolbar. Diagrams that were already inside the width budget — which is nearly all of them — are unchanged.
- The `The_step_delimiter_emitters_cap_the_bar_they_build` pin flipped deliberately: a 6,000-character unbroken step now takes the styled form instead of being truncated into an unreadable coloured bar. A sibling fact pins that a step that fits still takes the legacy form.

### Tests
- New `SequenceDiagramWidthTests` renders every width axis through **real Java PlantUML** (IKVM) and asserts the drawn `viewBox` stays under 4096 — the only discipline that can see a crop, since the source is always valid. Twenty-one facts, all of which fail against the previous release; the run also pins that a split fragment declares a strict subset of the participants and that the caller survives in every fragment.
- New `NoteAppearanceTests` (Playwright) drives the real controls and asserts on the **painted** SVG: the engine contract that `.className { MaximumWidth }` still works on a stereotyped note (the fact that speaks up if an engine bump breaks the feature silently), width and monospace round trips including byte-identical restoration of the source, the measured alignment win (an `AS` column spread of >20 px becomes <1 px), the button being absent on a note that already fits, survival across a headers toggle, and both bulk dropdowns.
- New `NoteAppearanceScriptTests` and `DiagramNoteWrapWidthTests`; `ReportToggleDefaultsResolverTests` and `ToggleDefaultsMarkupTests` gain the two new settings. The three source-rebuild paths were factored into one `composeNoteSource` so they cannot fork, and the pins that asserted the inlined chain now assert that single helper — a stronger contract than before.

### Documentation
- Wiki `PlantUML-Browser-Rendering.md` gains a *Note Appearance* section (the two controls, why monospace is the bigger half, what "full width" means, and the class-selector mechanics); `Large-Response-and-Diagram-Handling.md` gains a *Diagram Width* section with the measured per-emitter table, the raster-only nature of the 4096 limit, and what the budget deliberately does not reach; `Report-Configuration.md` gains the three new options.
- `plans/DIAGRAM_WIDTH_PLAN.md` and `plans/NOTE_WRAP_AND_WIDTH_PLAN.md` are committed as executed design records.
- Kronikol4J: report-output divergence widens (participant declarations, marker-note bodies, activity diagrams) plus the client-side script divergence for the appearance controls — ledger entry in its README.
...
### Fixed
- **The component diagram no longer draws itself wider than PlantUML can render** (user-reported against a real production report) — the architecture overview's ClickHouse edge listed thirty SQL operations on one line, and *the label never wrapped*. The two engines Kronikol renders through disagree about who wraps: the TeaVM JavaScript build used for `BrowserJs`/`NodeJs` honours `skinparam wrapWidth` on an arrow label, and **real Java PlantUML does not wrap arrow labels at all**. The same source therefore drew 840×863 inside the report and **6697×586** through real PlantUML — past the 4096-pixel `PLANTUML_LIMIT_SIZE` that plantuml.com and every default install crop at. The crop is silent: PlantUML keeps the top-left corner and discards the rest, so the diagram came back with the caller and the API and *every dependency node missing*, and no error anywhere to explain it. That is what a reader gets the moment the source leaves the report, which is precisely what the diagram context menu's *Copy / Open PlantUML source* is for. Fix: the generator puts the line breaks in itself rather than delegating them to a skinparam. An edge label over 100 characters is broken with `\n` escapes at `", "` boundaries where it can be — so a display line holds whole operations rather than half a table name — falling back to whitespace inside an item too long to stand alone, and hard-breaking only a token that carries no creole markup (`[[#anchor`, `<size:10>` and any `\` escape are never cut). The label's own `\n`s are structure, so each of them is wrapped independently and the stats label's three-line shape and its `[[#iflow-rel-…]]` link survive intact. Wrapping happens *before* the 2000-character statement cap, not after, because the `\n` escapes are two characters each and count toward the statement the parser measures. Reported diagram: **6697px → 1240px**, every node back on the canvas. Labels at or under 100 characters keep their exact bytes, and the browser report is unchanged either way because that engine re-wraps each line to `wrapWidth` regardless (840×863 → 840×899).
- **…and neither does a long dependency name** (found while fixing the above) — a participant's name is the same width axis and the *narrower* one. `skinparam wrapWidth` does apply to entity boxes, but it only breaks at whitespace, and the names that get long — hosts, fully-qualified type names, connection descriptors — have none. Measured against real PlantUML: a `<<system>>` rectangle grows at about 7.3 pixels per character and crops at around 528, `database`/`collections`/`queue` at around 570, the C4 `System()` macro at around 460, and the hexagon an `AI` dependency draws as at around **284**. Names over 80 characters are now broken the same way. The two `rectangle` forms close and reopen their creole bold markers on every line — `**a**\n**b**`, never `**a\nb**`, which loses the weight on every line *and* draws a literal `**` at the end (measured, not assumed) — while the plain shapes and the C4 macros take the breaks alone. Aliases are derived from the name rather than from the wrapped label, so no edge is disturbed.
- **SQL comments and string literals no longer masquerade as table names** (the `SELECT FROM the` in the same report) — the classifier's `FROM` and `INTO` table extractors are unanchored regular expressions with no lexer behind them, so the first `FROM`-looking phrase anywhere in the statement won. A query carrying `-- rolled up from the weekly aggregate` in front of its real clause was reported as reading a table called `the`, which showed up in the architecture overview beside twenty-eight genuine table names — and each such artefact is another distinct operation in the edge label, which is part of how that label grew long enough to overflow the canvas in the first place. The front of the statement had the mirror-image defect: the keyword matcher and all three prefix strippers are `^`-anchored, so a statement that *opened* with a comment classified as `Other` with no table at all — and at **Summarised** verbosity a call classified that way is dropped from the report entirely. `--` comments, `/* */` comments and `'…'` literals (with the ANSI `''` escape) are now removed in a single pass before classification, because the three forms nest into each other: a `--` inside a literal is not a comment and a quote inside a comment does not open one. Bracketed, double-quoted and backticked text is left alone — those delimit identifiers, and the table extractors read through them. Malformed input closes at the end of the statement rather than throwing. This affects classification only; the SQL a report displays is the SQL that ran, comments and all. Applies to every SQL tracker (SqlClient, Npgsql, MySqlConnector, Sqlite, Oracle, ClickHouse, Spanner, Dapper, EF Core).
- **A database response capped at `MaxResponseRows` gets pretty-printed again** (user-reported against the same production report, where 87 notes were affected) — a capped response is a *complete* JSON document with a `... (N more rows not shown)` footnote after it, and that footnote is what defeated the formatter. `TryFormatAsJson` parses the whole note body, so the footnote failed it; the fallback that exists for exactly this situation only knew the two markers a *cut* body carries (`…truncated (N chars total)` from the HTTP/tap capture cap, `…[bulk string truncated: …]` from the RESP decoder), so it declined too and the note fell through to the plain-text path. The result inside one scenario was that the uncapped response on one arrow painted as indented rows while the capped response on the next painted as a minified blob — in the reported report, 42 KB of ten ClickHouse rows across 87 columns wrapped into unreadable 120-character lines instead of 872 indented ones. The row cap now joins the marker set (`... (N more rows not shown)`, MongoDB's `... (N more documents not shown)`, a Spanner stream's `... (N more)`), and because what precedes a row cap is a whole document rather than a cut prefix, the body is re-serialized through the normal JSON path instead of the structural brace walker — so a capped payload indents **and drops nulls** exactly like the uncapped response beside it. Genuinely cut bodies still take the brace walker, since there is no document there to walk. Applies to every SQL tracker (SqlClient, Npgsql, MySqlConnector, Sqlite, Oracle, ClickHouse, Dapper, EF Core) and to MongoDB; Spanner's multi-row `FullRows` form emits one JSON object per line and is not a JSON document either way, so it stays plain text as before. Kronikol4J: report-output divergence (any capped database note's bytes).
- **…and those notes can now be switched to YAML** — the browser-side eligibility gate reconstructs a note's payload and runs `JSON.parse` over it, so the footnote excluded capped responses from the JSON ⇄ YAML toggle too, which is what made them the least readable notes in the report while being some of the widest. The exclusion was inherited from *cut* payloads, where there genuinely is no document to convert; a row cap is not that. The footnote is now split off before the gate and re-attached as a line under the emitted YAML — never dropped to buy the toggle, or the YAML view would show ten rows as though they were the whole result. The split is deliberately narrow: only the generator's own three wordings, only as the last payload line, so a payload line that merely resembles one is never eaten. Works through the per-note hover button, the scenario and report-wide dropdowns, and the zero-click `NotePayloadFormat.Yaml` default alike, and toggling back restores the original JSON bytes with the footnote exactly once. Kronikol4J: extends the standing `collapsible-notes-script.js` divergence (client-side script only; no report-output impact beyond the generator change above).

### Testing
- The regression tests for the first two live in a new `ComponentDiagramLabelWidthTests` in the IKVM project, and they render the generated diagram through **real Java PlantUML** and assert the drawn `viewBox` stays under 4096 — asserting on the *source* cannot see this class of bug at all, because the source is always valid. The reported diagram is rebuilt from its relationships verbatim (thirty ClickHouse operations included) and reproduces the 6697-pixel failure exactly; a companion fact reads the width back off the rasterised PNG, since the failure mode is a crop rather than an error. A theory covers all four participant shapes, and one fact pins the bold-marker trap by asserting every name run in the SVG carries `font-weight="bold"` and that no literal `**` reaches the output.
- The row-cap fix is pinned at both ends: a theory over the three footnote wordings plus a null-stripping parity fact and a non-JSON-body-with-a-footnote fact in `PlantUmlCreatorTests`, and a contract trap in `TrackingDbDataReaderTests` asserting the reader emits a complete JSON document with the footnote alone on the second line — the exact shape the note formatter splits on, so a reworded marker fails loudly instead of silently un-prettifying every capped note. A new E2E `CappedResponseNoteRenderingTests` drives a capped response through the **real** capture pipeline into the browser engine and asserts on the painted SVG. Verified red against the pre-fix formatter, where the same note paints the minified array.
- Asserting anything about a note's *line structure* needed a new shared helper, `PlaywrightTestBase.GetPaintedSvgLines`: PlantUML emits one `<text>` per word (spaces are their own elements), so `textContent` cannot tell one line from two that happen to concatenate — the display lines are rebuilt from the baselines the words share. Both new E2E classes assert through it.
- The YAML half is pinned at three levels: pure-function facts in `NoteYamlInternalsTests` (a theory over the three footnote wordings, a near-miss/not-last-line fact proving the split stays narrow, and one showing the payload only becomes eligible once the footnote is off), interaction facts in `NoteYamlToggleTests` (toggle to YAML over a fixture built through the real formatter, with the footnote asserted as its own painted line, plus a round trip proving the JSON bytes come back and the footnote is not duplicated), and a zero-click fact in `NoteFormatDefaultTests`, since the `NotePayloadFormat.Yaml` default reaches the notes through `setAllNoteFormats`/`_preProcessSource` rather than the hover button. `ClickNoteFormatButton` gained a note-index overload — the capped fixture's first note is the request's SQL text, which has no format button.
- Seven unit facts in `ComponentDiagramGeneratorTests` pin the label contract (broken into display lines, ordinary labels byte-identical, operations kept whole, the statement limit still honoured after wrapping, hard-breaking, markup never split, the stats label's line structure preserved); six in a new `ComponentDiagramNameWrappingTests` pin the participant contract; fourteen in a new `UnifiedSqlCommentStrippingTests` pin the SQL one, including the cases that must *not* change — `--` inside a literal, a doubled quote, quoted identifiers of all three kinds, `a-b` arithmetic, unterminated comments and literals, and the command text coming back verbatim.

### Documentation
- `Component-Diagrams.md` gains the label-wrapping and name-wrapping rules with the measured numbers and the engine disagreement behind them. The seven SQL integration pages gain the comment/literal note next to their verbosity tables.
- `Content-Formatting.md`'s formatting-pipeline list gains the row-cap rule, and `PlantUML-Browser-Rendering.md`'s eligibility section gains the row-capped-response paragraph: why the footnote used to exclude these notes, and how narrow the split that admits them is.
- New `plans/DIAGRAM_WIDTH_PLAN.md` records the rest of the audit that came out of this fix: nine other places Kronikol can draw past 4096 pixels, each with its measured pixels-per-character slope and crossing point — the sequence-diagram user-action label (≈730 characters), internal-flow activity diagrams (≈670, and the only family with no character cap *and* no `wrapWidth`), step-bar creole table cells (≈590 in one cell), assertion notes and user-action note bodies (≈760 and ≈600 unbroken characters, both bypassing `WrapUnbreakableRuns`), sequence participant names (≈650) — and the one that needs no pathological input at all: **about 28 participants crop a sequence diagram even with short names**, because there is a height guard but no width or participant-count guard. None of that is implemented; the plan is a record, not a green light.

## [3.0.82] - 2026-09-04

### Changed
- **Scenario descriptions get breathing room** (user-requested) — the `.scenario-description` div shipped with no CSS rule at all, so a description sat flush against the scenario summary above it and the Steps section below it. It now carries `margin: 1em 0`, matching the `.scenario` container's own vertical rhythm, at both emission sites (a plain scenario and the per-row detail panels of a grouped parameterized scenario). Text flow is deliberately unchanged — unlike `.feature-description`, no `white-space: pre-line` is applied, because scenario descriptions are prose hard-wrapped at the feature file's source width (measured across a real report: every description was multi-line source wrapping and none carried a blank-line paragraph break), so they read better reflowed than pinned to the author's wrap column.

### Fixed
- **Filter toggles no longer rewrite the embedded component diagram** (user-reported against a real production report) — hiding databases with the report-level "Databases Shown/Hidden" filter toggle stripped the database and collections dependency nodes (and every edge touching them) out of the embedded component diagram, silently under-reporting the system's dependencies in the architecture overview. The overview declares its dependency nodes with the same `database`/`collections` keywords the databases filter strips from scenario sequence diagrams, and every path that applies the filter selected all diagram containers: the first-render pre-process (which also made a databases-hidden *configured default* from 3.0.80 strip the overview on load, and covered the hide-then-open-panel order), the report-level databases toggle, and the assertions/steps toggles (whose queue builders re-apply the databases filter to every container they process). A new `isComponentDiagramContainer` guard exempts the overview from the whole filter pipeline — `_preProcessSource` returns its source untouched and every one of the six queue builders skips it, so no filter control ever rewrites *or re-renders* the component diagram. The three note-driven builders (details, headers, note format) were already inert for it in practice, but only because a component diagram carries no notes; they carry the guard too, so the exemption is structural rather than incidental and a component diagram that ever grows a note cannot start being rewritten. The standalone `Component.html` report embeds no filter script and was never affected. Tests: four Playwright facts in `ComponentDiagramDatabaseToggleTests` (toggle-after-render including a same-DOM-node pin proving the overview is not even re-rendered, hide-before-opening-the-panel, databases-hidden configured default, and steps/assertions re-application with databases hidden) over a new fixture whose scenario diagram and component diagram both carry database+collections participants, plus two unit facts pinning the guard into the generated script and asserting every one of the six queue builders carries it. Four pre-existing `ComponentDiagramReportTests` facts were sharpened while making this change: they searched the whole HTML file for the bare string `component-diagram-section`, which the shared report script now also carries as a CSS selector — two failed outright (one sliced from the script's occurrence in the `<head>` across the entire scenario list) and, more quietly, the section-ordering and section-present facts would have passed without any component diagram markup emitted at all. All four now assert on the emitted attributes (`class="component-diagram-section"` / `id="component-diagram"`), so they test markup rather than any mention of the name. Kronikol4J: extends the standing `collapsible-notes-script.js` divergence (client-side script only; no report-output impact).
...
### Added
- **Configurable default start states for every report toggle** — every toggle, radio group and stateful `<select>` in the generated HTML reports now has a configurable default, set in code alongside the other settings: `ReportConfigurationOptions.TestRunReportToggleDefaults` (a new `ReportToggleDefaults` group) applies to both reports, and `SpecificationsToggleDefaults` overrides per property for `Specifications.html` (inherit-unless-overridden — any property left `null` inherits the effective TestRunReport value; resolution is built-in ← TestRunReport group ← Specifications group, most specific wins). Twenty-four settings: the Details radio (`ReportDetailsState`), truncate lines (`TruncateLineCount` — one member per dropdown preset with explicit underlying values; the enum is now the single source of truth the markup builds its `<option>` list from, and an undefined cast like `(TruncateLineCount)37` fails generation naming the valid members), headers/assertions/steps/databases visibility, the note payload format (group value wins over the flat `NotePayloadFormat` option, which stays the simple both-reports knob — and is now the one way the two reports can start in different note formats), feature/scenario expansion (with the Expand All button labels seeded to the flip state), the diagram-type tab (`DiagramTabKind`, honoured where the view exists with the built-in fallback order), the Scenario Timeline and Component Diagram panels (mutually exclusive — timeline wins when both are configured visible), the dependency/category filter modes (`FilterCombinationMode`), and ten disclosure-section defaults (features summary, failure clusters, steps, diagrams, diagnostics, rules, background steps, raw PlantUML, the parameterized table's flat/grouped view, and the internal-flow Activity/Flame tab). Seeding is not cosmetic: the configured default drives the zero-click render pipeline — an `AssertionsShown = true` report draws assertion notes on first paint. Defaults govern the initial state only; after any interaction the page behaves exactly as today. A setting whose control a given report does not emit resolves normally and is inert. Everything is resolved once per report by the new public `ReportToggleDefaultsResolver` into a non-nullable `ResolvedToggleDefaults` record (its `BuiltIn` instance is now the single home of the literals previously duplicated across C# markup and JS globals); `GenerateHtmlReport` gains one optional `toggleDefaults` parameter (the legacy `notePayloadFormat` parameter stays and folds in when the record is absent), the merge renderer forwards merge-time options, and a same-run byte-identity pin proves unset config produces byte-identical output to every pre-existing call shape, for both report shapes, permanently.
- The five verbatim-duplicated scenario diagram-toolbar strings in `ReportGenerator` are factored into one `BuildScenarioDiagramToolbar` builder (same "built once — cannot drift" contract as the note-format select), proven byte-inert by a pre/post capture diff and pinned by the committed A/B test.

### Changed
- **URL-hash grammar** (affects all reports, not just configured ones): the hash now omits `depmode`/`catmode` when the mode equals the report's *configured* default (previously the built-in literals), `parse_url_hash` accepts both values symmetrically — `depmode=AND` and `catmode=OR`, previously ignored, are now valid, so a deep link from an AND-default report forces AND on an OR-default report and vice versa — and **Clear All** resets the filter modes to the configured defaults instead of hard-coded AND/OR (`clear_all_filters` previously silently un-configured a report). The single-match search reveal now also opens the scenario's Steps and Background Steps sections and an enclosing Rule section (a match could otherwise land invisibly inside a section a configured default closed — or one the reader closed).
- The truncate-lines fallbacks in `_setTruncateLines`/`_setScenarioTruncateLines` hard-coded `20`; they now route through the seeded default.
- Kronikol4J: parity break widens (options surface, `collapsible-notes-script.js` globals block tokens, the dependency/category/url-hash/export/search filter scripts, and toolbar/disclosure markup all diverge further; the port is pinned to 3.0.43 assets) — recorded in its README divergence ledger.
...
### Added
- **Step tables and doc strings now draw inside the diagram's step-delimiter bars** (user-requested) — a step like `Given the following muffins exist` never made sense in the diagram without its Gherkin data table. A step carrying a table, a doc string, or multi-line text now takes a styled bar form — `hnote across <<stepDelimiter>><<stepBody>>: text\n|= … |`, still one physical line so the report's hide-steps strip regex and every line-oriented consumer work unchanged — with the step text on the first line and the table drawn as a real bordered table (white on the black bar) or the doc string as indented lines beneath. Colours come from an injected `.stepBody` style (`BackgroundColor black`/`FontColor white`/`LineColor white`, emitted per diagram like the assertion style) rather than inline tags, because `<color:white>` styles only the first display line of a note and inline colour tags put the statement under the ~1,400-character coloured-bar crash cap; the tag-free form rides the 16,000-character note ceiling instead (measured on the shipped v1.2026.8beta1 engine: no crash and no size refusal at 15.9k characters, and past the `maxSvgSize` ceiling the engine refuses legibly rather than crashing — hiding steps recovers the diagram). Default on, no new option; bars without a table/doc string are emitted byte-for-byte as before, with one corner normalised: the live emitter now trims stray leading/trailing whitespace and CRs from the label, as the ingest emitter always did (no real step text is affected — the whole pinned suite is byte-identical). Wired everywhere the argument exists: the Reqnroll hooks pass `StepInfo.Table` + `MultilineText` (an outline row's table arrives with `<placeholders>` already substituted, so each row's diagram shows what that row actually received); the IL weaver's `TabularInputs`/`TabularOutputs` parameters draw as tables (named when a step has several); the tests-NDJSON `step` event's existing `table`/`docString` fields now reach the bar (`InteractionRecord` gains the same optional fields, additive); and the Cucumber Messages synthesizer fills the marker from the step it built.
- **Cucumber Messages: the pickle step's substituted `argument` is now parsed and preferred** over the authored Gherkin table/doc string (which keep raw `<placeholders>` for outline rows) — both the step list and the delimiter bar now show the substituted values, matching live Reqnroll runs; older streams without `argument` fall back to the authored form.

### Fixed
- **Multi-line step-marker text was invisible in bars** — the ingest format allows multi-line `text`, which the coloured bar folded into `\n` escapes and painted black-on-black (only the first display line got `<color:white>`). Multi-line text now switches to the styled form, where every line renders.
- **`kronikol query note` swallowed notes after a step bar and never listed event notes** — the one-line/block-note classifier looked for a spaced `" : "`, but a step bar's colon is unspaced (`#black:<color:white>`), so a bar flipped the reader into note mode and it consumed every line (real payload notes included) up to the next `end note`; and event notes open with `note<<eventNote>> right` — no space after `note` — which the `StartsWith("note ")` check missed entirely, so their payloads were never extracted. Both classifications fixed and pinned.
- Bar-content escaping is measured, not assumed: creole's `~` escape is inert in this note form on the shipped engine (`~|` leaves the pipe live), and the `<U+0022>`/`<U+005B>`/`<U+005C>`/`<U+005D>`/`<U+005F>` escapes substitute back *before* creole parsing (rebuilding the live marker — a cell containing `C:\new` could tear the row). Cells and doc-string lines are therefore neutralised with the escapes that verifiably substitute late (`<U+007C>` `<U+003C>` `<U+0026>` and line-opening `<U+002A>`/`<U+003D>`/`<U+0023>`), plus a zero-width space (`<U+200B>`) breaking doubled pair markers and following every backslash. Pinned by unit tests and a Playwright fact rendering the real emission on the real engine.
- Kronikol4J: report-output parity break extends — table/doc-string-carrying Reqnroll and ingested reports gain `<<stepBody>>` bars, the `.stepBody` style block, and (Cucumber ingest) substituted table values; the capture-side options surface is unchanged.
...
### Changed
- **PlantUML JS engine upgraded to a stock 1.2026.8beta1 master-snapshot build — sequence diagrams render 4–8× faster.** `TrackingDefaults.PlantUmlJsCdnBase` now points at `@v1.2026.8beta1-0e4f452` (a build of [plantuml/plantuml](https://github.com/plantuml/plantuml) master `0e4f452e` via `gradlew :plantuml-mit:npmPackage`, hosted on the same jsDelivr fork channel), replacing the patched `@v1.2026.6-patched` build. The snapshot carries the merged upstream performance campaign ([#2834](https://github.com/plantuml/plantuml/issues/2834) / [#2866](https://github.com/plantuml/plantuml/issues/2866): the LiveBoxes and SkinParam caches plus PRs #2835–#2839 and #2858–#2860) — measured on Kronikol-shaped diagrams with the `teoz` pragma every Kronikol diagram carries: a real report diagram 789 → 181 ms warm, a 500-arrow diagram 5.8 → 0.8 s, nested groups 653 → 94 ms. The engine file also shrinks 7.15 → 3.95 MB. The upgrade additionally brings browser `!theme` support ([#2848](https://github.com/plantuml/plantuml/pull/2848)), SVG background painting ([#2849](https://github.com/plantuml/plantuml/pull/2849)) and the Smetana/viz-fallback work ([#2852](https://github.com/plantuml/plantuml/pull/2852), [#2861](https://github.com/plantuml/plantuml/pull/2861)) into the shipped engine (none activated by Kronikol yet; `viz-global.js` is byte-identical to the previous pin and still loaded, so Graphviz-family layouts are unchanged).
- **The engine size-limit patch is retired — the limit is now passed at render time.** The new build is completely stock: upstream's `maxSvgSize` render option replaces the fork's patched-in 98304px limit, and every ES-module render call site (the worker host, the main-thread fallback, the Node renderer) passes `{ maxSvgSize: 98304 }`. The effective limit is unchanged; the wiring is pinned by unit tests on all three scripts, a Node-driver test asserting the option reaches the engine's `render` call, and a new E2E test rendering a diagram past the stock 8192px default. The "too large" refusal message now ends `(max 98304; override via the maxSvgSize option, …)`.
- **Statement-length limits re-measured against the new engine — all constants unchanged.** The 2,000-character message cap is exact on this build too (it is PlantUML's own); the TeaVM-build artifacts moved upward only (block openers now fail by engine stack overflow around 3,660–5,641 characters instead of a ~1,476 parse limit; the coloured-note-bar crash edge moved ~1,458 → ~4,124; note ceilings unchanged at ~16,370), so every constant stays safely under its cap. The real-engine boundary tests were re-pinned to the new behaviour, including a Teoz-compatible sequence-vs-class-fallback detector (Teoz — the default sequence engine since upstream 1.2026.7 — emits no CSS classes, so the tests now detect a real sequence render by the participant being drawn twice).
- Kronikol4J: report-output parity break — the CDN base URL baked into every BrowserJs report changes, and the new engine draws Teoz-styled sequence SVGs (no CSS classes, `<title>` elements on lifeline groups).
...
### Added
- **ClickHouse driver pairing packages** (user-requested design, replacing an interim reflection approach): `Kronikol.Extensions.ClickHouse.Client` (pairs with the `ClickHouse.Client` HTTP driver) and `Kronikol.Extensions.ClickHouse.Octonica` (pairs with `Octonica.ClickHouseClient`, native TCP). Each carries a compile-time driver reference and supplies an `IClickHouseDriverAdapter` (new hook on `ClickHouseTrackingOptions.DriverAdapter`) plus typed helpers — `WithClickHouseClientTestTracking` / `AddClickHouseClientTestTracking` and `WithOctonicaClickHouseTestTracking` / `AddOctonicaClickHouseTestTracking`. The core `Kronikol.Extensions.ClickHouse` stays driver-agnostic and works unpaired (with pre-3.0.74 rows-affected behaviour). Adapters run after the command executed and are fail-safe: an adapter failure degrades the logged label, never the user's query.

### Fixed
- **ClickHouse INSERTs no longer log "0 rows affected"** (user-reported, seen in BreakfastProvider's order-timings scenarios) — when paired with `Kronikol.Extensions.ClickHouse.Client`. `ClickHouse.Client`'s `ExecuteNonQuery` parses the HTTP response *body* for an int, and an INSERT's response body is empty — so the driver returns `0` even though rows were written, and the diagram note faithfully reproduced the lie. The pairing adapter reads the real count from the driver's `QueryStats.WrittenRows` (populated from the `X-ClickHouse-Summary` response header) whenever it is positive, falling back to the driver's return value otherwise (typed access; verified live against ClickHouse 25.8 with both drivers — Octonica's `ExecuteNonQuery` already returns real counts and its adapter passes them through).
- **SQL SELECT response payloads now show the actual returned rows by default** (user-requested; matches the HTTP-level integrations, which always showed real response bodies). `ResponseDetail` left unset now *follows the effective verbosity*: `FullRows` (JSON row preview, capped at `MaxResponseRows`) at Raw/Detailed, `RowCountAndColumns` (`"3 rows [station, …]"`) at Summarised; an explicitly set `ResponseDetail` pins the level regardless of verbosity, and phase verbosity overrides are respected (shared `SqlResponseDetailResolver`). Applies uniformly across ClickHouse, Npgsql, MySqlConnector, Oracle, SqlClient, Sqlite, Dapper, EF Core Relational and Spanner (its own `SpannerResponseDetail?` mirror). The `ResponseDetail` properties changed type to nullable enums; code that *reads* them may need a `?? default`.
- **The Sqlite wrapper never captured response payloads at all** — `TrackingSqliteCommand` predated response capture and logged empty responses for readers and scalars. It now wraps readers in `TrackingDbDataReader` and logs scalar values, identically to the other DbConnection-wrapping extensions (pinned against a real in-memory SQLite database).
- **EF Core dropped reader response content at Summarised verbosity even with `LogResponseContent = true`** — the direct logging path nulled all content at Summarised while the phase-variant builder and the shared `SqlDiagnosticTracker` only do so when `LogResponseContent` is off. Aligned; Summarised now shows the count+columns summary like every other integration.
- **`InternalsVisibleTo` was a silent no-op in four csprojs** (Spanner, Bigtable, MongoDB, MongoDB.V2): the property form inside `<PropertyGroup>` emits nothing — converted to the `<ItemGroup>` item form that actually generates the attribute.
- Test-coverage gaps closed around the affected area: the DbConnection-wrapping command decorators of Npgsql, MySqlConnector, Oracle and SqlClient had **zero** test coverage (only their DiagnosticSource trackers were tested) — each now has pipeline facts driving a row-bearing reader through the tracking command; Dapper, EF Core, ClickHouse, Sqlite and Spanner gained the same follows-verbosity/explicit-wins pins.
- Kronikol4J: capture-side only — no report renderer/format change, but captured diagram *content* from SQL integrations now differs (row payloads instead of summaries), and the new packages/options extend the port's missing options surface.
...
### Fixed
- **Deep search now rejoins event-note and assertion-note bodies too.** The 3.0.70 note-body rejoin (normalization rule 5b) only recognized `note left|right|over` openers, but the formatter also emits `note<<eventNote>> right` (event captures) and `hnote across <<assertionNote>>` (assertion notes) — text spanning an 80-char chunk or 120-char wrap break inside those note bodies was unfindable. The opener detection now covers every multi-line note form the formatter emits (`[hr]?note`, optional `<<class>>`, any side), with a colon guard so single-line forms (step delimiters, row markers) don't enter note mode, and a word-boundary so payload lines like "note leftovers…" don't either. Reference + C# + shipped JS updated together; new shared vectors pin all three.
- **A cross-language whitespace divergence could hide deep results.** Rule 5b's flush-left test used .NET `char.IsWhiteSpace`, which disagrees with the client's JS `/\s/` on exactly two characters (U+0085 NEL, U+FEFF) — a note-body continuation line starting with NEL was rejoined by the browser verify but not by the C# indexer, so the index lacked trigrams the verify text contained: a deep false negative. The C# side now implements the JS `/\s/` set exactly; the equivalence suite's reference transliteration and pseudo-corpus stress the two characters.
- **Positive queries can no longer lose a result to deep search.** The final deep result replaced the whole visible set for *every* query, but shallow matches the raw `data-search` text while deep matches it normalized — a step name literally containing `<b>` or `~-` could shallow-match and then be hidden when deep verification ran. Deep results are now authoritative only for advanced queries containing `!!` (the one class where more text can genuinely turn a match negative, and the only removal the design ever promised); all other queries are strictly add-only (`kronQueryCanRemove`, Jint-pinned).
- **A corrupt or undecodable index blob wedged the chip in "searching everything…" forever**: the worker's init promise had no rejection handler, and an unhandled rejection in a worker does not fire `worker.onerror`, so the pending query never answered. Init failures now post a `fatal` message — `indexState` becomes `error`, the chip hides, and shallow search is untouched (E2E-pinned with a deliberately corrupted blob).
- **The sparse row encoding was executed by zero tests.** The KSI1 varint-list row arm is only ever chosen at ≥17 docs and every fixture had ≤6 — while every production report with ≥17 scenarios runs it live. A 20-doc shared vector now pins the C# serializer's bytes and the shipped JS decoder on both encodings in one blob (61 bitset + 78 list rows).
- Test-coverage gaps closed after an adversarial audit of the 3.0.70 plan execution: the wrap-crossing E2E needle actually crosses the formatter's 120-char cut now (the fact asserts the source no longer contains the needle contiguously, then finds it via search — previously the needle sat wholly on one line and the fact passed with rejoin deleted); SQL text is exercised through the genuine `SqlDiagnosticTracker` capture shape (string method, database category) at unit and E2E level instead of riding in a JSON payload; arrow-label message text has its own E2E fact; example values are asserted through the decoded blob, not just the `data-search` attributes; the real `kronikol merge` command's output is asserted to carry a rebuilt index; and the JSON⇄YAML note toggle is operated for real before re-searching. The parameterized-row fact is renamed to what it asserts (group reveal): pinpointing *which row* matched a payload-only needle is hit-location UX — SEARCH_INDEX_PLAN Phase 2, deliberately unbuilt.
- 3.0.70 changelog correction: the `FullSearchIndex=false` delta list omitted the `.raw-plantuml <pre>` HTML-encoding fix (now listed there too). Kronikol4J: the 5b/whitespace normalization changes extend the 3.0.70 parity break — port against the regenerated `tests/shared-vectors/search-index-vectors.json`.

## [3.0.70] - 2026-08-31

### Added
- **The report search box now searches *everything* the report contains** (plans/SEARCH_INDEX_PLAN.md, executed in full). Until now the search deliberately covered only a per-scenario summary (names, steps, error messages, extracted participant names and URLs) — a string you could *see* in a note payload, a diagram message, an SQL statement or a flame chart was unfindable. Reports now embed a compact full-text index (`<script id="kron-search-index">`: hashed-trigram → scenario-bitset, FNV-1a-32 over the normalized corpus, adaptive bitset/delta-varint rows, gzip+base64) over every diagram's full source, note payloads and headers, flame-chart span/source/marker text, and parameterized example values. **Measured: a 147 MB-of-diagram-text monster report gains a 1.07 MB blob** (typical reports gain far less; `tools/search-bench/` holds the full measurement record).
  - **Same search box, same syntax.** Shallow results still apply instantly on every keystroke; deep results merge in behind them, announced by a small chip — *"+N more found in payloads & diagrams"*, *"no additional matches"*, or *"results refined (+N/−M)"* for negated queries, where deep search is authoritative (the full corpus can newly match a `!!` term and *remove* a shallow result). Deep matching runs the **existing** matchers (`advanced-search.js` / legacy tokens) over the reassembled corpus in a Web Worker, so `&&`/`||`/`!!`/`@tag`/`$status` compose unchanged and the index is only ever an accelerator: candidates are confirmed with the same `includes()` semantics, making hash collisions harmless. Cold worst-case (broad term, everything decompressed and verified, first use) measured at 70 ms on the E2E large fixture and budgeted in the contention-scaled perf guard (`DeepQueryMs`); worker memory is LRU-bounded and nothing touches browser storage.
  - **Users search for what they *see*, so index and verify normalize identically** (shared `normalizeForSearch`, pinned across C#, the shipped report JS and Kronikol4J by generated cross-language vectors in `tests/shared-vectors/`): the diagram formatter's 80-char note chunking, 120-char unbreakable-run wrapping, arrow-label `\n` escapes, creole escapes and markup tags are all normalized away, so a long token or URL physically split across diagram lines is found as one contiguous string. Validated against real `PlantUmlCreator` output (`tools/search-bench/formatter-probe`), which also surfaced two rules the design would otherwise have missed (arrow-label literal `\n` breaks; line-leading `#`/`=` creole escapes).
  - **`ReportConfigurationOptions.FullSearchIndex`** (default `true`) opts out; with it off the only output changes vs 3.0.69 are the example-values coverage fix, the `<pre>` encoding fix (both below) and the always-included client scripts. Generation cost is engineered per plan §5.1 — per-distinct-text memoization shared between both HTML reports, thread-pool prewarm overlapping HTML building, two-pass flat-array serialization — and measured (M3): **+0.15–0.35 s on a 147 MB-corpus report that takes ~3.1 s to generate** (within run-to-run noise; phase breakdown in `tools/search-bench/README.md`); typical reports are unmeasurable. Merged reports rebuild the index automatically, including flame text extracted from precomputed fragments. Kronikol4J: **not yet ported** — the always-included client scripts (`report-decompress-helper.js`, `report-search-index.js`), the `<pre>` encoding fix and (default-on) the index blob all change the emitted HTML, so byte-parity against Java output breaks until the port catches up; the shared vectors + `SearchIndexBuilder` layout are the porting spec, and an unported report degrades cleanly (the deep-search client no-ops when the blob is absent).

### Fixed
- **Parameterized example values were unsearchable** unless echoed in the display name: `ExampleValues`/`ExampleFlatValues` are rendered as table cells but appeared in neither `data-search` nor `data-row-search`. They are now in both (and in plain-rendered outline scenarios), so the instant search finds them too.
- **InlineSvg and internal-flow reports shipped calls to a `decompressGzipBase64` global that never existed** in those modes (defined only inside the BrowserJs render script; the context-menu and internal-flow popup scripts call it regardless). One shared, always-included `report-decompress-helper.js` now defines it everywhere — including the standalone component-diagram page — and the render/flame scripts drop their private duplicates.
- **Plain-img reports interpolated raw diagram source into the `.raw-plantuml <pre>` unencoded**, so a payload containing `<` produced malformed markup (and broke the `textContent` round-trip deep search reads the source back through). Both emission sites now HTML-encode.

## [3.0.69] - 2026-08-30

### Changed
- **`kronikol query` is 14–42% faster on large reports** (plans/QUERY_PERF_PLAN.md, executed in full). ~25% of users have reports over 50 MB; on a 142.9 MB worst-case benchmark (24,000 interaction entries, every body distinct so dedup wins nothing — harness committed at `tools/query-bench/` with the measurement protocol and full before/after record) the same-session medians moved: `summary` 1.39 → 1.19 s, `values --path '$.status'` 2.89 → 1.95 s, `grep --number --count` 4.53 → 2.64 s. **No output byte, command surface or report format changed** — the entire existing CLI suite is the byte-identical pin, and perf properties are asserted on deterministic observables, never wall-clock. Kronikol4J: none (the tool is .NET-only). Four independently revertible fixes:
...
- **The render-worker perf guard measures up to three times and passes on one clean attempt** (`087be03`, recorded here on 2026-09-22). Contention alone had outrun every calibrated budget on a saturated two-core CI runner (`ReadyMs` 7208 against 4500, blocked 23.9 s of 33.1 s) without any regression to catch; the structural asserts still hold on every attempt and a sustained breach fails all three with an explicit message. Superseded in 3.0.69 by budgets that scale with the measured contention.
...
- **YAML-state filter-survival coverage completed.** The wiki claimed the toggle survives the assertion/step/database filters but only the steps filter was tested; assertion- and database-filter survival tests are now pinned (green — the shared `applyNoteFormats` pipeline already preserved the state). Remaining deliberately-uncovered paths are recorded in `plans/PLANS_STATUS.md`.
