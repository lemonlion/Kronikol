...
| `LLM_FIRST_PLAN.md` *(new, 2026-09-12)* | 🟡 **In progress — green-lit, one release per milestone.** Successor audit to `LLM_FRIENDLY_PLAN`: 18 read-only lanes, 144 findings, 108 adversarially verified — **47 did not survive as written**, so every row is a claim to re-verify. **M0 and M1 shipped in 3.1.0; M2 (`Failures.md` tells the truth) in 3.2.0; M3 (you find out from the log that the run failed) in 3.3.0** — A1–A9, Q1 and Q2 in full, including the two defects that only appeared when the digest was run against a real report (the `Thrown at` frame landed on xUnit v3's own source-linked assertion PDBs, and a `MessageQueue` send printed its payload in the Call column). **M4–M8 remain**, in order: the schema tells the truth about the file beside it → every address the tool prints, the tool accepts → a merged report is a real run → one question, one command → discoverability (in-repo only, publish nothing). §16 is the implementation log | 3.1.0, 3.2.0, 3.3.0 |
