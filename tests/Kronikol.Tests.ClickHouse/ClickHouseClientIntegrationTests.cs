...
// All supported clients name their connection type `ClickHouseConnection`; alias via global::
...
using ClickHouseDriverConnection = global::ClickHouse.Driver.ADO.ClickHouseConnection;
...
public class ClickHouseDriverAdapterTests
{
    [Fact]
    public void Resolve_prefers_positive_WrittenRows_over_driver_result()
    {
        // Same lineage and same quirk as ClickHouse.Client: the driver's return value is a
        // body-parsing artifact (0 for INSERT); the X-ClickHouse-Summary stats are authoritative.
        var stats = new global::ClickHouse.Driver.ADO.QueryStats(0, 0, WrittenRows: 3, 0, 0, 0, 0, 0);
        Assert.Equal(3, ClickHouseDriverAdapter.Resolve(stats, driverResult: 0));
        Assert.Equal(3, ClickHouseDriverAdapter.Resolve(stats, driverResult: 7));
    }

    [Fact]
    public void Resolve_falls_back_to_driver_result_when_stats_missing_or_zero()
    {
        Assert.Equal(5, ClickHouseDriverAdapter.Resolve(null, driverResult: 5));
        var zeroStats = new global::ClickHouse.Driver.ADO.QueryStats(0, 0, WrittenRows: 0, 0, 0, 0, 0, 0);
        Assert.Equal(0, ClickHouseDriverAdapter.Resolve(zeroStats, driverResult: 0));
    }

    [Fact]
    public void Resolve_clamps_counts_beyond_int_range()
    {
        var hugeStats = new global::ClickHouse.Driver.ADO.QueryStats(0, 0, WrittenRows: long.MaxValue, 0, 0, 0, 0, 0);
        Assert.Equal(int.MaxValue, ClickHouseDriverAdapter.Resolve(hugeStats, driverResult: 0));
    }

    [Fact]
    public void Unexecuted_ClickHouseDriver_command_resolves_to_driver_result()
    {
        using var conn = new ClickHouseDriverConnection();
        using var cmd = conn.CreateCommand();
        Assert.Equal(9, ClickHouseDriverAdapter.Instance.ResolveRowsAffected(cmd, 9));
    }

    [Fact]
    public void Non_ClickHouseDriver_command_resolves_to_driver_result()
    {
        Assert.Equal(4, ClickHouseDriverAdapter.Instance.ResolveRowsAffected(new FakeDbCommand(), 4));
    }
}

public class ClickHouseDriverPairingExtensionTests
{
    [Fact]
    public void WithClickHouseDriverTestTracking_pairs_the_ClickHouseDriver_adapter()
    {
        using var inner = new ClickHouseDriverConnection();
        var options = new ClickHouseTrackingOptions();
        using var tracked = inner.WithClickHouseDriverTestTracking(options);

        Assert.Same(ClickHouseDriverAdapter.Instance, options.DriverAdapter);
        Assert.IsType<TrackingClickHouseConnection>(tracked);
    }
    ...
    [Fact]
    public void IsClickHouseConnection_true_for_ClickHouseDriver()
    {
        using var conn = new ClickHouseDriverConnection();
        Assert.True(Kronikol.Extensions.ClickHouse.ClickHouseServiceCollectionExtensions.IsClickHouseConnection(conn));
    }
    ...
    [Fact]
    public void AddClickHouseDriverTestTracking_decorates_and_pairs()
    {
        var services = new ServiceCollection();
        services.AddTransient<DbConnection>(_ => new ClickHouseDriverConnection());
