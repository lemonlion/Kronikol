...
namespace Kronikol.Tests.MySqlConnector;
...
/// </summary>
public class TrackingMySqlCommandResponseDetailTests : IDisposable
{
    private readonly string _testId = Guid.NewGuid().ToString();
    private readonly MySqlTrackingOptions _options;
    private readonly TrackingMySqlConnection _trackingConnection;

    public TrackingMySqlCommandResponseDetailTests()
    {
        TrackingComponentRegistry.Clear();
        _options = new MySqlTrackingOptions { CurrentTestInfoFetcher = () => ("TestMethod", _testId) };
        _trackingConnection = new TrackingMySqlConnection(
            new MySqlConnection("Server=localhost;Database=analytics"), _options);
    }
    ...
    {
        using var cmd = new TrackingMySqlCommand(new RowReaderFakeDbCommand(), _trackingConnection);
