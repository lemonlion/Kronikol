...
using Kronikol.Extensions.Oracle;
using Kronikol.Sql;
using Kronikol.Tracking;
using Oracle.ManagedDataAccess.Client;
using Xunit;

namespace Kronikol.Tests.Oracle;
...
/// </summary>
public class TrackingOracleCommandResponseDetailTests : IDisposable
{
    private readonly string _testId = Guid.NewGuid().ToString();
    private readonly OracleTrackingOptions _options;
    private readonly TrackingOracleConnection _trackingConnection;

    public TrackingOracleCommandResponseDetailTests()
    {
        TrackingComponentRegistry.Clear();
        _options = new OracleTrackingOptions { CurrentTestInfoFetcher = () => ("TestMethod", _testId) };
        _trackingConnection = new TrackingOracleConnection(
            new OracleConnection("User Id=app;Password=pw;Data Source=localhost/XE"), _options);
    }
    ...
    {
        using var cmd = new TrackingOracleCommand(new RowReaderFakeDbCommand(), _trackingConnection);
