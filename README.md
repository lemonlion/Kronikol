...
Tracks interactions between your test caller, your Service Under Test (SUT), and its dependencies — including HTTP calls, Azure Cosmos DB operations, SQL queries (via EF Core), Redis commands, events/messages, and arbitrary method calls — then converts them into sequence diagrams embedded in searchable HTML reports and YAML specification files.  Method flow within the SUT itself is turned into activity diagrams.  And the combination is turned into flame diagrams.  All the flows combined are turned into a C4 Component Diagram for your service.  A Scenario Timeline diagram allows you to visually compare the execution time of your services.  A cross-run history ledger committed in your repository says, on every scenario, whether a failure is a regression, has been failing since a particular run, or flips - as a sparkline and verdict in the report, a line in `Failures.md`, and `$flaky` in the search box.

Input data sets (eg InlineData/MemberData/ClassData for xUnit, and equivalents in NUnit, TUnit & ReqNRoll & LightBDD) are automatically turned into dynamic clickable tables showing you the correct diagrams for each set of inputs.  Data is all collapsible, truncatable and toggleable at view time (including a hover toggle on diagram notes that flips JSON payloads into YAML so multi-line strings like SQL queries read with their real line breaks — plus JSON/YAML toolbar dropdowns that convert every note at once, report-wide or per scenario, and a `NotePayloadFormat` option / `kronikol ingest --note-format yaml` to start whole reports in YAML), allowing you to see from higher level to lower level at the click of a button.  Background steps render inline with each scenario's own steps, in one combined step list.  Gherkin data tables and doc strings draw inside the diagram's step delimiter bars too (with a Scenario Outline's placeholders substituted per example row), so a step like "Given the following muffins exist" carries its data right where the flow shows it.  Also contains features for tracking your assertions from FluentAssertions/AwesomeAssertions/TUnit assertions and displaying them in the reports in plain english.  The report search box searches *everything* the report contains — request/response payloads, headers, SQL, diagram and flame-chart text — via a compact embedded full-text index, on top of the instant scenario-level search.  
...
| **ClickHouse** | `Kronikol.Extensions.ClickHouse` | Tracks ClickHouse operations via `DbConnection` wrapping — works with both `ClickHouse.Client` and `Octonica.ClickHouseClient`. Classifies ClickHouse SQL including `ALTER TABLE … UPDATE/DELETE` mutations, `OPTIMIZE`, `RENAME`, `ATTACH/DETACH` | [![NuGet Version](https://img.shields.io/nuget/v/Kronikol.Extensions.ClickHouse)](https://www.nuget.org/packages/Kronikol.Extensions.ClickHouse) |
| **ClickHouse (ClickHouse.Client pairing)** | `Kronikol.Extensions.ClickHouse.Client` | Pairs the ClickHouse extension with the `ClickHouse.Client` HTTP driver: accurate INSERT rows-affected counts (from the driver's `QueryStats`) plus typed wrap/DI helpers | [![NuGet Version](https://img.shields.io/nuget/v/Kronikol.Extensions.ClickHouse.Client)](https://www.nuget.org/packages/Kronikol.Extensions.ClickHouse.Client) |
| **ClickHouse (ClickHouse.Driver pairing)** | `Kronikol.Extensions.ClickHouse.Driver` | Pairs the ClickHouse extension with `ClickHouse.Driver`, the official ClickHouse .NET client: accurate INSERT rows-affected counts (from the driver's `QueryStats`) plus typed wrap/DI helpers | [![NuGet Version](https://img.shields.io/nuget/v/Kronikol.Extensions.ClickHouse.Driver)](https://www.nuget.org/packages/Kronikol.Extensions.ClickHouse.Driver) |
| **ClickHouse (Octonica pairing)** | `Kronikol.Extensions.ClickHouse.Octonica` | Pairs the ClickHouse extension with the `Octonica.ClickHouseClient` native-TCP driver: typed wrap/DI helpers and a compile-time driver adapter | [![NuGet Version](https://img.shields.io/nuget/v/Kronikol.Extensions.ClickHouse.Octonica)](https://www.nuget.org/packages/Kronikol.Extensions.ClickHouse.Octonica) |
...
| **OTLP Tap + Export** | `Kronikol.Extensions.Otlp` | Both OTLP directions: an OTLP/HTTP receiver-tee that maps the spans services already export to Kronikol interactions (exact test attribution by W3C trace id), and an exporter (`OtlpExporter`, `OtlpExportSink`, `kronikol export`) that pushes Kronikol's captures to Tempo/Jaeger/any collector as OTel spans | [![NuGet Version](https://img.shields.io/nuget/v/Kronikol.Extensions.Otlp)](https://www.nuget.org/packages/Kronikol.Extensions.Otlp) |
