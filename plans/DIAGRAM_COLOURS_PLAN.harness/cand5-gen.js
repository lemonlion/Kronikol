'use strict';
// Second P3 audit: which form of a captured backslash a step bar can carry on both engines. The bar is one
// statement whose display lines are joined by a literal \n; the shipped escaper puts <U+200B> after each backslash,
// which the Java engine reads as `\<` (it paints "U+200B>"). Writes cand5.json for cand-probe.js.
...
require('fs').writeFileSync(require('path').join(__dirname, 'cand5.json'), JSON.stringify(cases));
