...
| D6 | Move the engine pin to npm 1.2026.8 now, or wait for 1.2026.9. Keep `viz-global.js` or drop it | **Taken 2026-09-26 as recommended** (the owner asked for `ENGINE_PIN_PLAN.md` in full). Move now and keep `viz-global.js`. The point of the move is leaving a personal fork tag for the published, immutable package route. 1.2026.9 has no date, and its trailing-zero trimming forces a second golden re-pin whichever way this goes | 1.8 |
...
| D17 | The Teoz gate's number: "does it change a real report's render time enough for anyone to notice? If not, it is craft rather than product" | Set it from 1.8's perf budget once the pin has moved. **The baseline exists (3.31.1):** a warm 200-step arrow-heavy render through the worker, median 247 ms per contention stretch (198 to 373 over 9 runs), budget 1,500; the ladder's 267 ms with its 10% rep noise stays the finer instrument (`ENGINE_PIN_PLAN.md` §10.2). The threshold itself is still the owner's to set | §5 |
...
| D22 | **`flow` nesting** (`FLOW_NESTING_PLAN.md` S2): the green light, the parent rule, and whether a request never answered says so (the plan's Q1). The rule, R4: a call is indented under the innermost call still waiting for its answer whose service made it, or, when the call comes from another caller, on whose trace it arrived | Yes to all three. Measured on 7,183 real requests: R4 nests 60% of them, all one level deep, and its indentation never points at a line that is not the parent. The plain "innermost open call" rule gets 74 parents wrong (parallel fan-out as a staircase four deep, a lost response capturing later calls, a background query capturing the test's call), and the caller-only rule's indentation misleads 961 times. It costs 3.9% more bytes. S1 (1.13) needs no decision. **Taken 2026-09-26:** yes to all three, as recommended | 10.0 |
| D23 | **Fleet evidence** (`FLEET_EVIDENCE_PLAN.md`): the green light; whether it waits for the launch (rule 9) or runs beside stages 6 to 12 as tracks A and D for the owner's own company; the party kinds (five, with `unknown` the default and `real` never inferred, plan Q1); whether the service card is on by default (Q2); JSON-only specs and overlays until a number asks for YAML (Q3) | After the launch, as 14.11 to 14.15, with two exceptions taken now: the two contract fields (`ServiceId`, `Parties`) are decided in `PLATFORM_FOUNDATIONS_PLAN` §4.7 before 14.5 freezes the format (rule 6: free before, a format version after), and `graph` (S2, tool-only, one `VerbTable` line) may be pulled forward whenever track A is idle (rule 8). Take the plan's Q1 to Q3 as recommended: five kinds, card on, JSON only | 14.11 |
...
| 1.6 | **Two diagram defects from the theme audit that do not depend on themes.** `<color:gray>` renders `#808080` at 3.87 to 1 on the default note, below AA: replace it with a computed value ("worth doing whether or not themes ever ship"). Hovering a link permanently blacks out its text, because the restore writes `#000000`. And make `PlantUmlTheme` honest: a theme set under BrowserJs is silently ignored, so say so in a diagnostic until 6.1 makes it work (true on the pin the audit measured; from 3.0.76 the same setting left the report without diagrams, `DIAGRAM_COLOURS_PLAN.md` F21). **First release shipped 3.29.6** (`DIAGRAM_COLOURS_PLAN.md` S3a, S4, S5: the renderers answer the engine's script loads at once, captured text escapes the icon, emoji and sprite syntax, the Node renderer writes XML); **the two colours and the diagnostic shipped 3.30.0** (S1: header lines `#686868`, 5.46 and 4.51 to 1 on the two note fills; S2: a link rests in the ink of the text around it, and blue text that is no link keeps its colour; S3b: `DiagnosticKind.OptionNotApplied`); **the escapes the plan deferred shipped 3.30.1** (captured text PlantUML's preprocessor or creole would act on is written as code points, §12.3); **an audit shipped as 3.30.2** (§12.4: a 3.30.1 redaction regression, request labels, step-bar bodies, the render-error placeholder, the tool's note readers, a missing component-diagram fact and the docs); **a second audit shipped as 3.31.2** (§12.5: every kind of captured text drawn on both engines; a carriage return let the Java engine run `!include`, and step bars drew `U+200B>` for every backslash under Java; leftovers in Appendix C) | PlantUML Theme Audit. PLAN: neither was re-checked at HEAD | `THEME_PLAN` §2.3 bundles the contrast fix into the theme work; it stands alone |
...
| 1.12 | **#100** Every assertion note and step marker gets an internal-flow segment that no arrow links, since 3.15.1. `InternalFlowSegmentBuilder` makes a segment of every request record that has a time, and 3.15.1 began stamping a time on marker records too. Such segments are 55% to 68% of the segment map on BreakfastProvider's published reports; without them its xUnit report goes from 8.0 to 4.8 MB and `domContentLoaded` from 128 to 94 ms | #100. RUN on 2026-09-25: every captured record dumped on the consumer's xUnit lane at 3.29.6 (1,602 of the 1,605 unlinked segments are markers) and on the in-repo ReqNRoll example (65 of 65); `INTERNAL_FLOW_BLOB_PLAN.md` F8 | A bug fix, so a patch, with no decision needed. Planned as R1 of the P5 plan and released before 1.9, because it touches no script and halves the map that 1.9 then compresses. It changes report output: one ledger entry. Added 2026-09-25 |
| 1.13 | **Three `flow` defects** (`FLOW_NESTING_PLAN.md` S1). Under a filter or a step address, `kronikol query flow` prints the header of every step that made a call, with nothing under it: `flow s26 --errors-only` on a passing BreakfastProvider scenario prints five empty headers, which read as five steps that made no calls (they made 38), and `flow s26/4` prints the headers of steps 0 to 3 above step 4. An annotation recorded after a scenario's last call is never printed, though `annotations` lists it. A dictionary is built and never read. **Shipped 3.30.3**, with two more found beside them (plan F13, F14): `flow` and `trace` ignored `--count`, and count now, while `compare` ignored it too and refuses it now; and `flow` on a scenario that made no calls blamed a filter | RUN on 2026-09-26 against 3.29.6 (BreakfastProvider's ReqNRoll lane and a probe report). READ: `QueryCommand.Narrative.cs:467-527` | A bug fix, so a patch, with no decision needed. Placed by rule 1. Tool-only: no report bytes, no ledger entry. It first pins `flow`'s pairing and whole output, which nothing tests today. Track A; S2 of the same plan is 10.0. Added 2026-09-26 |
...
| # | Item | Bump | Notes |
|---|---|---|---|
| 10.0 | **`flow` shows which call ran inside which** (`FLOW_NESTING_PLAN.md` S2). A call is indented under the call that was waiting for its answer when it was made, when that call's service made it or it arrived on that call's trace; a line whose parent the indentation cannot show names it (`inside s26/i8`), so a filtered view still says which request a query belonged to. It came out of the owner's question whether a Mermaid rendering would help an LLM: measured, it would not, and its one gain over `flow` was this | minor | D22 taken 2026-09-26. **First in the stage:** before 10.1, because the server wraps the verbs, and before 10.2, whose cold agent must "name the interaction that broke", which nesting does when a 502 has the 503 inside it. Before 14.3 by rule 6: the conformance corpus pins `flow`'s text. Tool-only: no report bytes, no ledger entry. It touches `QueryCommand.Narrative.cs`, one `VerbTable` line and both skill copies, so it **may be pulled forward** to any point when no other track A stage is in flight (rule 8: small and measured). Added 2026-09-26. **Shipped 3.31.0 (2026-09-26).** Found beside it, open for the owner (plan Q4, Q7, Q8): an ingested run loses nesting to ingest's own order (a record without a timestamp sorts first, and call-tree order puts a delivery after its parent's response), and Kronikol4J's unreleased HTTP adapters write a call after the calls it made, so a Java report nests nothing |
...
| 14.11 | **`FLEET_EVIDENCE_PLAN` S1, the two contract fields.** `ReportConfigurationOptions.ServiceId` and `Parties` (name, id, kind: `test`, `underTest`, `real`, `double`, `unknown`), a `parties` block in the report and the id on the history run line; `services` gains a `kind` column | minor | Needs D23. **Before 14.5** by rule 6: the block is a section of the options file F4 carries, and the freeze must hold it. Track D with one track A file. The block is emitted only when declared, so every existing corpus stays byte-identical (plan §7.5 measures it first). Kronikol4J ledger entry. Added 2026-09-26 |
| 14.12 | **S2 and S3: `graph` and `coverage`.** `graph` prints the participants and edges a component diagram is drawn from, with id and kind, in 1 to 2 KB; `coverage` reads a JSON OpenAPI (and AsyncAPI) spec against the run: exercised, unexercised, unspecified, undeclared statuses, required properties never present | minor each | `graph` needs nothing and **may be pulled forward** to any point when no other track A stage edits `VerbTable` (rule 8, as 10.0 was). `coverage` is the answer to the plan's F1: a report describes the tested surface, and only the spec says how much of the service that is. JSON only (Q3): the tool has no YAML reader and no third-party dependency (RUN) |
| 14.13 | **S4, the service card.** `Service.md` and `service.json` at run end beside `Failures.md`, behind `GenerateServiceCard`: identity, exposes (against the spec), consumes (by what the evidence is of: the real service, a double, unknown), messages, flows, where the tests live at the precision the run has, history, redaction attestation, next verbs; about 8 KB, byte-stable when behaviour is | minor | After 14.11 and 14.12: a card that states a stub's answers as a dependency's contract is the plan's F2. Both skill copies and the emitted `CLAUDE.md` name it first on a green run. Kronikol4J ledger entry |
| 14.14 | **S5 and S6, the fleet.** `kronikol fleet index` over cards, reports or history views with a JSON overlay, writing `Fleet.md` and `fleet.json` (services, edges joined on id, unresolved names, no-evidence services); `who-handles`; `fleet graph`; the address form `<serviceId>@<run>/<address>`; `find_reports` (10.1) over a fleet root | minor | Files on the `kronikol-history` branch, the dashboard plan's distribution (§4.6 there); no server, no cross-source verdict arithmetic (§4.8 there). The company map the owner asked for is `Fleet.md`; index-first search is what keeps a twenty-service fleet from costing twenty cards a question (F5) |
| 14.15 | **S7, the gate.** An agent given only a fleet repository (BreakfastProvider's real report plus a synthetic second service through `kronikol ingest`, JSON specs with one operation unexercised on purpose) names the owning service, its handler namespace, the tests to extend, every caller and the untested sibling, without opening a report, under a token ceiling measured on the first cold run | none | Run cold before and after, as 10.2 is. It is what says whether the ladder answers the owner's question or only describes it |
...
|---|---|---|---|
| A. History and the tool | 1.1, 1.2, 1.13, 4, 7, 9, 10 | `Kronikol.Tool/`, `History/`, `VerbTable`, both skill copies | B, C, D. **Never two of its own stages at once** |
...
#93 → 1.4 · #94 → 1.3 · #96 → 9.2 and §6 · #97 → 1.11, D21 · #100 → 1.12
...
`REPORT_QUERY` → §6 · `LLM_FRIENDLY` (M3.1) → 10.1 · `FLOW_NESTING` → 1.13, 10.0 (D22) · `FLEET_EVIDENCE` → 14.11 to 14.15, D23 · `TEOZ_PERF` → 1.8, §5 · `PERF_CI`, `SMETANA_PERF` →
...
|---|---|---|---|---|
| `FLEET_EVIDENCE_PLAN` (2026-09-26) | OPEN, NGL. Needs D23 | none | 14.11 to 14.15, D23 | Every service's test run as queryable metadata for an LLM: the evidence layer under the specs (intent) and a derived company map (navigation). Two contract fields the report lacks and that would make the evidence lie to a model: which party is the service under test and whether each other party was real or a double (F2), and a service id that joins across reports (F3); both decided before 14.5 freezes the format. Then `graph`, `coverage` against a JSON OpenAPI or AsyncAPI spec, a per-run `Service.md` card of about 8 KB, `kronikol fleet index` and `who-handles` over files on the history branches, a service locator on addresses, and a cold-agent gate. Checked today: no spec-reading code in `src/`, the tool has no third-party dependency, the ledger's `caller>service` edges are display names, only Cucumber-ingested runs carry feature-file locations, `InteractionRecord` omits nulls |
...
| `DIAGRAM_COLOURS_PLAN` | Q7: `FocusDeEmphasis.LightGray`, the default, paints at 1.47 to 1. A hung engine is never recovered (the worker keeps its stuck state, a Node batch its process). `OptionNotApplied` for the other options `BrowserJs` ignores (`PlantUmlImageFormat`, `LocalDiagramRenderer`, `LocalDiagramImageDirectory`, `PlantUmlServerBaseUrl`); the theme entry does not consult `DiagramFormat`, and what it should say under Mermaid is unchecked. Q10: OpenIconic for a user's own `<&icon>`. Step, test, assertion, span and action text keep their creole by design (`<b>`, `**`, `//`, `[[…]]` style or link). A backslash in an arrow label is read (`\n` breaks it, `\t` is a tab, Java reads `\<`); a path cannot hold one, only a GraphQL operation name, a thrown status or an ingested method, and escaping it means building the label from escaped parts. A component diagram's method and participant names are written as given. The port's four script defects (its ledger) | Q7 goes with 6.1 or 11.2. Hung-engine recovery goes with 1.8 (`ENGINE_PIN_PLAN.md` §7 points here). The rest open, unscheduled (plan §6, §12.4 and §12.5) |
| Found by `DIAGRAM_COLOURS_PLAN` §12.5, not its own | No generated report passes relationship stats to its component diagram (`ReportGenerator`, `MergeableReportRenderer`, `ComponentDiagramReportGenerator`), and none has since `74b9763c` (2.0.92-beta) removed the stats path. The P50/P95/P99 lines, the error rate and the `[[#iflow-rel-…]]` links that open the relationship popup exist only when a consumer computes the stats and passes the diagram in | The owner's choice: restore them in reports (a minor) or keep them to the API, as `Component-Diagrams` now says |
