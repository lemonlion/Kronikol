...
- **Cucumber phase mapping** — exact Given→Setup / When·Then→Action wiring for the BDD adapter (Phase 5). **Parity note (K-C1):** Cucumber-JVM already emits the whole Gherkin model on `--plugin message:target/messages.ndjson` — the same Cucumber Messages protocol `Kronikol.Ingestion.Cucumber` reads (`kronikol ingest --cucumber-messages`). **The importer is the adapter**: rather than a bespoke `Given`/`When`/`Then` hook set, the Java port can point the run at the messages file and get feature description, rule, background, keywords, `PickleStep.type` (the phase an `And` inherits), data tables, doc strings, outline example values, statuses, exceptions, retries and attachments for free — with the `kronikol-test-id` attachment joining the scenario to its captured traffic. A native hook adapter is then only needed for live/in-process step bars.
...
## Appendix C — Proxy-tap / ingestion / Playwright contracts (added with .NET 3.0.44)

The .NET codebase now ships three generic assets the Java port must mirror byte-for-byte on the wire (see the wiki pages *Integration ProxyTap Extension*, *Ingesting External Captures*, *Integration Playwright*, *Capture-Time Redaction*):

1. **Proxy-tap topology (`kronikol4j-proxytap`, new module).** A transparent HTTP tee (`ProxyTap` + `ProxyTapOptions`) for backends that cannot be instrumented. Identity resolution order: `test-tracking-current-test-name`/`-current-test-id` → configured fallback headers → W3C `traceparent` trace id. Re-inject the four `TestTrackingHttpHeaders` (`test-tracking-current-test-name`, `-current-test-id`, `-caller-name`, `-trace-id`) on the forwarded request when missing; honour `test-tracking-ignore`; forward bytes untouched (hop-by-hop headers stripped, `Host` rewritten); emit server/client spans on an `ActivitySource`-equivalent (`Kronikol.ProxyTap`) and re-parent the forwarded `traceparent`; decode gzip/deflate/br for capture only; cap bodies (`…truncated (N chars total)` marker); **redact the secret denylist at capture** (`authorization`, `proxy-authorization`, `cookie`, `set-cookie`, `x-api-key`, `x-auth-token`, `api-key`, `x-amz-security-token`, `x-goog-api-key`); unattributed requests forwarded but not captured unless opted in (`CaptureUnattributedRequests` + optional fixed `FallbackTestId`). Options shape = the `ProxyTapOptions` record in `src/Kronikol.Extensions.ProxyTap/ProxyTapOptions.cs`. Java servlet/Jetty/undertow-based tee is acceptable; semantics must match.
1a. **Database taps (`kronikol4j-tcptap`, new module; added with .NET 3.0.45 — wiki page *Integration TcpTap Extension*).** The TCP counterpart of the proxy tap: a byte-for-byte tee (`TcpTap` + `TcpTapOptions`, `RedisTap`/`RedisTapOptions`, `MongoTap`/`MongoTapOptions`, `TcpTapHostedService`) that a service's Redis or MongoDB client is pointed at, so those hops render exactly like the in-process `kronikol4j` Redis/MongoDB extensions with nothing changed inside the service. Options shape = `src/Kronikol.Extensions.TcpTap/TcpTapOptions.cs` (listen host/port with `0` = ephemeral exposed as `boundPort`, forward host/port, caller/service names, dependency category, `Verbosity` `Summarised|Detailed|Raw`, `BodyCapBytes` 65536, `CaptureReplies`, sink, phase, `FallbackTestName`/`FallbackTestId`, `IdentityResolver` supplier returning name/id or null, `EmitActivities` on a `Kronikol.TcpTap` source, `KeyRedaction`/`ValueRedaction`, `ChannelCapacity` 1024, `MaxBufferedBytes` 8 MiB, `ReadBufferBytes` 32 KiB, `AcceptBacklog`, connect/drain timeouts, `DecoderFactory`) plus the protocol option classes (`ExcludedCommands`, `ExcludedKeyPrefixes`, `DefaultDatabase`, `CapturePubSub`; `ExcludedCommands`, `TrackGetMore`, `LogFilterText`, `LogResponseContent`, `MaxResponseDocuments`, `DocumentRedaction`). A Netty or NIO tee is acceptable; the semantics must match exactly: **forward downstream before queueing the copy**, bounded drop-on-full queue with per-direction counters, half-close propagated, decoding on its own task, a decoder error caught and confined to one connection while forwarding continues, auth commands hard-excluded whatever the options say (Redis `AUTH`/`HELLO`/`RESET`; MongoDB `saslStart`/`saslContinue`/`saslSupportedMechs`/`authenticate`/`getnonce`/`copydbsaslstart`/`copydbgetnonce`/`createUser`/`updateUser`), and the connection string never recorded. Wire coverage: RESP2 and RESP3 (one self-describing parser; commands are arrays of bulk strings, replies FIFO per connection, unsolicited pushes skipped, `SELECT n` followed, hit/miss decided on the reply); MongoDB OP_MSG (flag bits, section kinds 0 and 1 folded into the body, replies by `responseTo`, `moreToCome` handling), with OP_QUERY/OP_REPLY passed through and recorded never and OP_COMPRESSED passed through undecoded and reported once. The NDJSON must be byte-compatible: `method` labels (`Get (Hit)`, `Find ← Trial`, `Insert (×3) → Trial`, `Aggregate ($match, $group) ← x`), URIs (`redis://db{n}/{key}` with multi-key commands comma-joined, `mongodb:///{db}/{coll}` from the command's `$db`), reply notes (`n=…, nModified=…` plus up to `MaxResponseDocuments` pretty-printed `cursor.firstBatch` documents), status mapping (Redis `OK` / 500 + error text; Mongo 200 / 500 + `errmsg`) and the real command/reply timestamps on the request and response records.
2. **NDJSON capture format + `ingest` (`kronikol4j-cli ingest`, `InteractionRecord` in core).** One JSON object per request *or* response, camelCase, matching `TestRunReport.json`'s `httpInteraction` shape plus `testId`, `testName`, `dependencyCategory`, `callerDependencyCategory`, `phase`, `metaType`, `activityTraceId`, `activitySpanId`, `trackingIgnore`, `capturedBy`, `durationMs` (a capturer's own measurement, believed over the timestamp delta; the writer carries it from .NET 3.27.4), `kind` (`ui` / `step` / `assertion` / `marker`) and, for `kind: marker` (.NET 3.29.0, one record per override half as the in-process store holds it), `markerKind` (`Custom`/`Row`/`Step`/`Assertion`/`Phase`), `plantUml` (verbatim, buffering newlines included) and `markerEnd` (nulls omitted, unknown properties ignored). A `marker` record restores exactly one override half; a capture whose `Step` markers cover a scenario draws no tests-file bars for it. Companion tests NDJSON: `event` (start|step|end), `testId`, `testName`, `feature`, `timestamp`, `status`, `durationMs`, `error`, `text`, `keyword`. Pipeline semantics: read → clear store → replay in timestamp order → normalise test names from the tests file → synthesise features (feature per `feature`, scenario per `testId`; tests seen only in interactions default to Passed) → generate. Status vocabulary mapping in `FeatureSynthesizer.MapStatus`. Non-UUID ids hash (MD5 → UUID) so pairs still match; 32-hex W3C trace ids parse as UUIDs directly.
...
### OTLP receiver-tee and the wire/span merge (added with .NET 3.0.45, K-A4)

6. **OTLP topology (`kronikol4j-otlptap`, new module).** Mirror `Kronikol.Extensions.Otlp`: an OTLP/HTTP receiver on `POST /v1/traces` accepting `application/x-protobuf` and `application/json` (+ gzip), optional byte-for-byte forward to a real collector with its status/body relayed, otherwise `200` with an **empty** `ExportTraceServiceResponse` (zero bytes / `{}`); shared-secret `ExpectedHeaders` → `401`; request-size cap → `413`; bounded drop-on-full mapping queue (ack before mapping); counters for received/mapped/ignored/dropped/unauthenticated/forward-failed. Java may use its Servlet/Jetty stack and `io.opentelemetry.proto` if convenient — the .NET package hand-rolls the protobuf reader only to stay dependency-free; the *wire* behaviour is what must match. `SpanToInteractionMapper` semantics are the contract: deprecated **and** stable semconv (`db.system`↔`db.system.name`, `db.statement`↔`db.query.text`, `db.operation`↔`db.operation.name`, `db.mongodb.collection`↔`db.collection.name`, `db.name`/`db.redis.database_index`↔`db.namespace`, `net.peer.*`/`peer.service`↔`server.address`/`server.port`, `http.method`↔`http.request.method`, `http.url`↔`url.full`, `http.status_code`↔`http.response.status_code`); Redis → command verb + optional `(Hit)`/`(Miss)`, `redis://db{n}/{key}`, category `Redis`; MongoDB → the MongoDB extension's directional labels (`Find ← Trial`, `Insert → Trial`, `FindAndModify ↔ Trial`, no `(×N)` — not derivable from a span), `mongodb:///{db}/{collection}`, category `MongoDB`; other `db.system` → operation or first SQL verb, `{system}://{server}/{namespace}`, category `BigQuery`/`PostgreSQL`/`MySQL`/`SqlServer`/… else `Database`; HTTP client spans → method + `url.full`; `messaging`/`rpc` opt-in; server spans ignored unless `IncludeServerSpans`; ERROR status → `500` + status message as the response body; `testId` = the trace id, `activityTraceId`/`activitySpanId` always set; `ServiceNameMap` resolves participants (`peer.service` → `host:port` → `host` → system name).
7. **Wire/span merge (`ingest --merge-duplicates`).** `InteractionMerger` in core: two records merge when caller, service, the method **verb** (first word) and the last URI path segment agree and their `[start, end]` intervals overlap by ≥ `MergeOverlapThreshold` (default 0.8) of the shorter; greedy by best overlap, strictly one-to-one (bursts pair off, never collapse). Span identity (`testId`/`traceId`/`activityTraceId`/`activitySpanId`) wins, wire fidelity (`content`/`statusCode`/label) wins, and the merged request gains the pseudo-header `x-kronikol-captured-by: wire + span`. New optional `capturedBy` field (`wire` | `span`) on the interaction record and the log entry, with inference as the fallback (span id and no content → span-like; content and no span id → wire-like; ambiguous → never merged).
...
### Attachments, widened records, capitalisation, window attribution (added with .NET 3.0.45)

Four wire-level additions to item 2 above; a Cucumber-JVM run emitting `--plugin message:…` covers most of the
structure for free, but a plain JUnit reporter reaches the same fidelity through the tests NDJSON alone.

1. **`attachment` event** — `{event:"attachment", testId, name, path, mediaType?, step?, timestamp}`. `path` is
   absolute, relative to the ingest's attachments base, **or a `http`/`https` URL** (rendered as a link, never
   copied — and never handed to the path APIs). `mediaType` decides inline rendering (`image/*` inline with a
   lightbox) and beats the extension sniff (`.png .jpg .jpeg .gif .webp .svg .avif .bmp`). `step` is the
   0-based index of the top-level step; absent → scenario-level; an index that no longer resolves falls back to
   the scenario rather than losing the artefact. `FileAttachment` gains an optional third positional
   `mediaType`, carried in `TestRunReport.json`/`.xml`/`.yml`, the JSON Schema, the XSD **and** the mergeable
   report (`kronikol merge`). `IngestRequest.CleanAttachments` empties `<reports>/attachments/` first.
2. **Widened `start`/`step`/`end`** — `start`: `featureDescription`, `description`, `rule`, `tags[]`,
   ...
   first row = header, ragged rows padded, header-only ignored), `stackTrace`, `bypassReason`; `end`:
   `stackTrace`. New model field `Scenario.Description`. Tags follow the ReqNRoll conventions (`@category:`,
   `@endpoint:`, happy-path aliases; a tag present on **every** scenario of a feature also becomes the
   feature's label). An explicit `background: true` step suppresses the heuristic `BackgroundStepsDetector`
   entirely, which also declines to run when no step carries a keyword.
3. **Capitalisation (`StepText`)** — one helper, ported as one function. Upper-case the first *letter* of a
   step or assertion label that carries **no keyword**, after skipping whitespace and the marker glyphs
   `✓ ✗ ⚠ • - – — *`; leave a label starting with `" ' ( [ {` or the typographic quotes verbatim; never touch a
   keyword step; culture-invariant (`toUpperCase` with `Locale.ROOT`), Unicode-aware, idempotent. Default on
   (`CapitaliseStepText`; `ingest --no-capitalise`). Applied once over the finished model so every data format
   agrees, and to the diagram's `<<stepDelimiter>>` bars (keyword-less form) and `<<assertionNote>>` text, so
   picture and step list read alike. Leftovers are reported as a `StepsNotStartingWithCapital` diagnostic.
4. **Window attribution + phases** — `ingest --attribute-by-window [fallbackId]`: a record with no `testId`
   (or the capturer's fallback marker) joins the test whose `[start, end]` window contains its timestamp;
   overlapping windows → the **latest-started**; a tie → the first window in the file; no window → left alone
...
(`--strict` restores the throw), each scenario's diagram production and each render call are isolated so one
failure yields a `<<renderError>>` placeholder for that scenario only, each report output is isolated the same
way, and everything is reported through structured `IngestResult.Diagnostics`
(`{kind, message, scenarioId?}`) rather than only as footer text.
