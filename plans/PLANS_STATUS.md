...
| `DOCUMENT_OWNERSHIP_PLAN.md` *(2026-09-15)* | 🟡 **Green-lit, in progress as 3.19.0.** A document a scenario wrote is the scenario's: an identity-less document operation (a detached hosted service claiming, retrying and failing an outbox row) is attributed to the scenario that last wrote the document, per call, with the provenance `DocumentOwner`; expiry treats it like an inherited identity. Cosmos first; Mongo and Spanner if their captures expose the key. | 3.19.0 |
