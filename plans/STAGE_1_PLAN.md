...
| P4. The engine pin. **Written:** [`ENGINE_PIN_PLAN.md`](ENGINE_PIN_PLAN.md) (2026-09-22, not green-lit; measured: npm 1.2026.8 is SVG-identical to the pinned build on the corpus, so no golden re-pin; second pass: the browser verifies the hash itself in three engines, so the check ships no digest code; third pass 2026-09-25: Kronikol's own emitter output is byte-identical on both builds too, a damaged V8 code cache crashes node so the Node cache gains a checksum, and a `viz-global.js` mismatch drops Graphviz only) | 1.8 | engine constants, the emitter's statement limits (S0), Node renderer, worker host, render-bench (track B) | two patches: S0 (a long linked request label keeps its diagram in the worker), then the pin; golden re-pin only if bytes move | none for S0; D6 for the rest |
