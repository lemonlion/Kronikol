...
| P4. The engine pin. **Green-lit 2026-09-26, S0 shipped as 3.30.4** (a link's text capped at 350 characters on the request arrow and the component edge, measured in the worker): [`ENGINE_PIN_PLAN.md`](ENGINE_PIN_PLAN.md) (2026-09-22; measured: npm 1.2026.8 is SVG-identical to the pinned build on the corpus, so no golden re-pin; second pass: the browser verifies the hash itself in three engines, so the check ships no digest code; third pass 2026-09-25: Kronikol's own emitter output is byte-identical on both builds too, a damaged V8 code cache crashes node so the Node cache gains a checksum, and a `viz-global.js` mismatch drops Graphviz only) | 1.8 | engine constants, the emitter's statement limits (S0), Node renderer, worker host, render-bench (track B) | two patches: S0 (a long linked request label keeps its diagram in the worker; 3.30.4), then the pin; golden re-pin only if bytes move | none for S0; D6 for the rest (taken 2026-09-26 as recommended) |
