        ...
        for (var i = 1; i < args.Count; i++)
        {
            ...
            switch (arg)
            {
                ...
                case "--alternating-runs":
                    if (Next() is not { } alternating) return 2;
                    if (!int.TryParse(alternating, NumberStyles.Integer, CultureInfo.InvariantCulture, out var parsedAlternating) || parsedAlternating < 1)
                    {
                    ...
                    parsed.AlternatingRuns = parsedAlternating;
                    break;
                case "--count-runs":
                    if (Next() is not { } countRuns) return 2;
                    if (!int.TryParse(countRuns, NumberStyles.Integer, CultureInfo.InvariantCulture, out var parsedCountRuns) || parsedCountRuns < 1)
                    {
                    ...
                    parsed.CountRuns = parsedCountRuns;
                    break;
                case "--degraded-by":
                    if (Next() is not { } degradedBy) return 2;
                    ...
                    {
                        error.WriteLine("--degraded-by takes a factor: a run whose passing scenarios took this many times their usual is degraded, the report's HistoryDegradedBy (default 2.0; 0 switches it off).");
                        return 2;
                    }
                    parsed.DegradedBy = parsedDegradedBy;
                    break;
